...
export interface CharacterDomainRepository {
  upsertTraitState(input: NewCharacterTraitStateRecord): Promise<CharacterTraitStateRecord>;
  getTraitStates(characterId: string): Promise<CharacterTraitStateRecord[]>;
  deleteTraitStates(characterId: string): Promise<void>;

  createTraitHistory(input: NewCharacterTraitHistoryRecord): Promise<CharacterTraitHistoryRecord>;
  getTraitHistory(characterId: string): Promise<CharacterTraitHistoryRecord[]>;

  upsertEmotionState(input: NewCharacterEmotionStateRecord): Promise<CharacterEmotionStateRecord>;
  ...
  updateGoal(id: string, characterId: string, input: Partial<NewCharacterGoalRecord>): Promise<CharacterGoalRecord>;
  getGoals(characterId: string): Promise<CharacterGoalRecord[]>;

  upsertInfluence(characterId: string, input: Partial<NewCharacterInfluenceRecord>): Promise<CharacterInfluenceRecord>;
  getInfluence(characterId: string): Promise<CharacterInfluenceRecord | null>;

  createRelationship(input: NewCharacterRelationshipRecord): Promise<CharacterRelationshipRecord>;
  updateRelationship(characterId: string, targetCharacterId: string, input: Partial<NewCharacterRelationshipRecord>): Promise<CharacterRelationshipRecord>;
  getRelationships(characterId: string): Promise<CharacterRelationshipRecord[]>;

  createDomainEvent(input: NewCharacterDomainEventRecord): Promise<CharacterDomainEventRecord>;
