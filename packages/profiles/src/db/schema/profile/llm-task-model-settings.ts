...
export const llmTaskModelSettings = profileSchema.table("llm_task_model_settings", {
  id: primaryId(), userId: varchar("user_id", { length: 128 }).notNull(), householdId: varchar("household_id", { length: 128 }).notNull().references(() => households.id, { onDelete: "cascade" }),
  provider: varchar("provider", { length: 40 }).notNull().default("openrouter"), taskType: varchar("task_type", { length: 60 }).notNull(), modelId: text("model_id").notNull(),
  reasoningLevel: varchar("reasoning_level", { length: 20 }).notNull().default("medium"), temperature: real("temperature").notNull().default(0.8), maxOutputTokens: integer("max_output_tokens").notNull().default(1800), enabled: boolean("enabled").notNull().default(true), ...timestampColumns,
}, (table) => [
