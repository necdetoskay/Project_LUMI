...
import {
  ...
  EMOTION_DIMENSIONS,
  NEED_TYPES,
  CHARACTER_LIFECYCLE_STAGES,
  CHARACTER_SUBTYPES,
  type CharacterSubtype,
  type CharacterLifecycleStage,
  type TraitDimension,
  type NeedType,
} from "./types";
...
export function validateTraitVector(vector: TraitVector): void {
  if (!vector || typeof vector !== "object") {
    throw new ValidationError(
      "INVALID_TRAIT_VECTOR",
      "Trait vector must be a non-null object",
      "traits",
    );
  }
  for (const [key, value] of Object.entries(vector)) {
    if (!(TRAIT_DIMENSIONS as readonly string[]).includes(key)) {
      throw new ValidationError(
        "UNKNOWN_TRAIT_DIMENSION",
        `Unknown trait dimension: ${key}`,
        key,
      );
    }
    if (typeof value !== "number" || Number.isNaN(value)) {
      throw new ValidationError(
        "INVALID_TRAIT_VALUE",
        `Trait value for ${key} must be a number`,
        key,
      );
    }
    if (value < 0 || value > 1) {
      throw new ValidationError(
        "TRAIT_OUT_OF_RANGE",
        `Trait value for ${key} must be between 0 and 1`,
        key,
      );
    }
  }
...
export function validateEmotionVector(vector: EmotionVector): void {
  if (!vector || typeof vector !== "object") {
    throw new ValidationError(
      ...
      "Emotion vector must be a non-null object",
      "emotions",
    );
  }
  for (const [key, value] of Object.entries(vector)) {
    if (!(EMOTION_DIMENSIONS as readonly string[]).includes(key)) {
      throw new ValidationError(
        "UNKNOWN_EMOTION_DIMENSION",
        `Unknown emotion dimension: ${key}`,
        key,
      );
    }
    if (typeof value !== "number" || Number.isNaN(value)) {
      throw new ValidationError(
        ...
        `Emotion value for ${key} must be a number`,
        key,
      );
    }
    if (value < 0 || value > 1) {
      throw new ValidationError(
        ...
        `Emotion value for ${key} must be between 0 and 1`,
        key,
      );
    }
  }
...
export function validateNeeds(needs: NeedState[]): void {
  ...
  for (const need of needs) {
    if (!(NEED_TYPES as readonly string[]).includes(need.needType)) {
      throw new ValidationError(
        "UNKNOWN_NEED_TYPE",
        `Unknown need type: ${need.needType}`,
        "needType",
      );
    }
    ...
    if (need.value < 0 || need.value > 1) {
      throw new ValidationError(
        "NEED_OUT_OF_RANGE",
        `Need value must be between 0 and 1`,
        "value",
      );
    }
    if (
      typeof need.decay !== "number" ||
      Number.isNaN(need.decay) ||
      need.decay < 0 ||
      need.decay > 1
    ) {
      throw new ValidationError(
        "INVALID_NEED_DECAY",
        `Need decay must be a number between 0 and 1`,
        "decay",
      );
    }
  }
...
export function validateGoals(goals: GoalState[]): void {
  ...
  for (const goal of goals) {
    if (!(NEED_TYPES as readonly string[]).includes(goal.needType)) {
      throw new ValidationError(
        "UNKNOWN_GOAL_NEED_TYPE",
        `Unknown need type in goal: ${goal.needType}`,
        "needType",
      );
    }
    if (!goal.description || goal.description.length > 500) {
      throw new ValidationError(
        "INVALID_GOAL_DESCRIPTION",
        "Goal description must be 1-500 characters",
        "description",
      );
    }
    if (!["active", "completed", "failed", "abandoned"].includes(goal.status)) {
      throw new ValidationError(
        "INVALID_GOAL_STATUS",
        "Goal status must be active/completed/failed/abandoned",
        "status",
      );
    }
  }
...
export function validateInfluenceVector(influence: InfluenceVector): void {
  const dims: (keyof InfluenceVector)[] = [
    "emotional",
    "social",
    "cultural",
    "educational",
  ...
  for (const dim of dims) {
    ...
    if (typeof value !== "number" || Number.isNaN(value)) {
      throw new ValidationError(
        "INVALID_INFLUENCE_VALUE",
        `Influence ${dim} must be a number`,
        dim,
      );
    }
    if (value < 0 || value > 1) {
      throw new ValidationError(
        "INFLUENCE_OUT_OF_RANGE",
        `Influence ${dim} must be between 0 and 1`,
        dim,
      );
    }
  }
  ...
  for (const rel of relationships) {
    ...
    if (seen.has(rel.targetCharacterId)) {
      throw new ValidationError(
        ...
        `Duplicate relationship to ${rel.targetCharacterId}`,
        "targetCharacterId",
      );
    }
    ...
    if (typeof rel.trust !== "number" || rel.trust < 0 || rel.trust > 1) {
      throw new ValidationError(
        "RELATIONSHIP_TRUST_RANGE",
        "Trust must be between 0 and 1",
        "trust",
      );
    }
    if (
      typeof rel.affinity !== "number" ||
      rel.affinity < 0 ||
      rel.affinity > 1
    ) {
      throw new ValidationError(
        "RELATIONSHIP_AFFINITY_RANGE",
        "Affinity must be between 0 and 1",
        "affinity",
      );
    }
    if (
      typeof rel.familiarity !== "number" ||
      rel.familiarity < 0 ||
      rel.familiarity > 1
    ) {
      throw new ValidationError(
        "RELATIONSHIP_FAMILIARITY_RANGE",
...
export function validateTraitDelta(delta: TraitDeltaEntry): void {
  if (!(TRAIT_DIMENSIONS as readonly string[]).includes(delta.dimension)) {
    throw new ValidationError(
      "INVALID_TRAIT_DELTA_DIMENSION",
      `Unknown trait dimension: ${delta.dimension}`,
      "dimension",
    );
  }
  if (
    delta.oldValue !== undefined &&
    (typeof delta.oldValue !== "number" ||
      Number.isNaN(delta.oldValue) ||
      delta.oldValue < 0 ||
      delta.oldValue > 1)
  ) {
    throw new ValidationError(
      "INVALID_TRAIT_DELTA_OLD_VALUE",
      "Old value must be a number between 0 and 1",
      "oldValue",
    );
  }
  if (
    typeof delta.newValue !== "number" ||
    Number.isNaN(delta.newValue) ||
    delta.newValue < 0 ||
    delta.newValue > 1
  ) {
    throw new ValidationError(
      "INVALID_TRAIT_DELTA_NEW_VALUE",
      "New value must be a number between 0 and 1",
      "newValue",
    );
  }
  if (!delta.evidence || delta.evidence.trim().length < 1) {
    throw new ValidationError(
      "TRAIT_DELTA_REQUIRES_EVIDENCE",
      "Trait delta requires evidence",
      "evidence",
    );
  }
  if (delta.evidence.length > 500) {
    throw new ValidationError(
      "TRAIT_DELTA_EVIDENCE_TOO_LONG",
      "Evidence must be 500 characters or less",
      "evidence",
    );
  }
}
...
export function resolveTraitDeltaAgainstState(
  ...
  if (
    delta.oldValue !== undefined &&
    Math.abs(delta.oldValue - currentValue) > TRAIT_OLD_VALUE_TOLERANCE
  ) {
    throw new ValidationError(
...
export function validateCharacterSubtype(value: string): CharacterSubtype {
  if (
    !(CHARACTER_SUBTYPES as readonly string[]).includes(
      value as CharacterSubtype,
    )
  ) {
    throw new ValidationError(
      "INVALID_CHARACTER_SUBTYPE",
      `Character subtype must be one of: ${CHARACTER_SUBTYPES.join(", ")}`,
      "characterSubtype",
    );
  }
...
export function validateCharacterLifecycleStage(
  value: string,
): CharacterLifecycleStage {
  if (
    !(CHARACTER_LIFECYCLE_STAGES as readonly string[]).includes(
      value as CharacterLifecycleStage,
    )
  ) {
    throw new ValidationError(
      "INVALID_LIFECYCLE_STAGE",
      `Lifecycle stage must be one of: ${CHARACTER_LIFECYCLE_STAGES.join(", ")}`,
      "lifecycleStage",
    );
  }
