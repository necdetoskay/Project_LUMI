...
describe("generation context policy", () => {
  it("keeps character onboarding focused on child and current creation choices", () => {
    ...
    expect(getGenerationContextSectionPolicy("character_onboarding", "world_state")).toBeNull();
  });
  ...
  it("reserves richer continuity context for story generation", () => {
    ...
    expect(getGenerationContextSectionPolicy("story_generation", "character_state")?.priority).toBe(
      "required",
    );
    expect(getGenerationContextSectionPolicy("story_generation", "relevant_memories")?.priority).toBe(
      "medium",
    );
  });
    ...
    expect(() =>
      assertGenerationContextPolicy({
        ...
        sections: [
          ...
          { section: "child_personalization", priority: "required", maxTokens: 60 },
        ],
      }),
