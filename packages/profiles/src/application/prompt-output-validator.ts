...
function validateNode(value: unknown, schema: JsonSchema, path: string, issues: string[]) {
  ...
  if (Array.isArray(value)) {
    ...
    if (schema.items && typeof schema.items === "object") value.forEach((item, index) => validateNode(item, schema.items as JsonSchema, `${path}[${index}]`, issues));
  }

  if (value && typeof value === "object" && !Array.isArray(value)) {
    const record = value as Record<string, unknown>;
    const required = Array.isArray(schema.required) ? schema.required.filter((key): key is string => typeof key === "string") : [];
    for (const key of required) if (!(key in record)) issues.push(`${path}.${key}: required`);
    const properties = schema.properties && typeof schema.properties === "object" ? schema.properties as Record<string, JsonSchema> : {};
    for (const [key, childSchema] of Object.entries(properties)) if (key in record) validateNode(record[key], childSchema, `${path}.${key}`, issues);
  }
}

export function parseAndValidatePromptOutput(raw: string, schema: JsonSchema): unknown {
  const cleaned = raw.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "");
  let value: unknown;
  try { value = JSON.parse(cleaned); } catch { throw new Error("LLM_OUTPUT_INVALID_JSON"); }
  const issues: string[] = [];
