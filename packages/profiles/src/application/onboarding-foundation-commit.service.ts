...
function inferGenesisArchetype(origin: string): GenesisArchetype {
  ...
  const matches: Array<[RegExp, GenesisArchetype]> = [
    ...
    [/son kalan|last known/, "last_known"],
...
export async function getOnboardingFoundationGenerationProvenance(
  cycleId: string,
): Promise<{ genesis?: FoundationProvenance; saga?: FoundationProvenance }> {
  const db = getProfileDb();
  const traces = await db
    .select({
      taskType: aiGenerationTraces.taskType,
      promptKey: aiGenerationTraces.promptKey,
      promptVersion: aiGenerationTraces.promptVersion,
      provider: aiGenerationTraces.provider,
      modelId: aiGenerationTraces.modelId,
      id: aiGenerationTraces.id,
      createdAt: aiGenerationTraces.createdAt,
    })
    .from(aiGenerationTraces)
    .where(
      and(
        eq(aiGenerationTraces.creationCycleId, cycleId),
        eq(aiGenerationTraces.validationStatus, "valid"),
      ),
    )
  ...
  const mapTrace = (
    taskType: string,
    intent: string,
  ): FoundationProvenance | undefined => {
...
export function buildOnboardingFoundationRecord(
  ...
  const genesisProvenance =
    input.genesisProvenance ??
    fallbackProvenance(
      input,
      "character_genesis",
      "character_onboarding.origin_suggestions",
    );
  const sagaProvenance =
    input.sagaProvenance ??
    fallbackProvenance(
      input,
      "saga_foundation",
  ...
  const record: CharacterFoundationRecord = {
    ...
    sagaCanon: {
      ...
      possibleTransformations: input.evidence.saga.themes.map(
        (theme) =>
          `${theme} temasının karakterde kalıcı bir dönüşüme yol açması`,
      ),
      revealLayers: [
        {
          id: "reveal-1",
          order: 1,
          label: "İlk örüntü",
          reveal: `İlk ipuçları ${input.evidence.origin.storyHook} olayının tek başına olmadığını gösterir.`,
          prerequisites: ["İlk yerel bağlantıların kurulması"],
        },
        {
          id: "reveal-2",
          order: 2,
          label: "Köken bağı",
          reveal: `Köken ile ${input.evidence.saga.title} arasında doğrudan bir bağ vardır.`,
          prerequisites: ["Birden fazla bağımsız ipucunun doğrulanması"],
        },
        {
          id: "reveal-3",
          order: 3,
          label: "Derin gerçek",
          reveal: deepTruth,
          prerequisites: ["Core Saga geç evresi ve yeterli kanıt"],
        },
      ],
...
export async function getCharacterFoundationByCharacterId(
  ...
  const [row] = await db
    .select({ foundation: characterFoundations.foundation })
    .from(characterFoundations)
    .where(eq(characterFoundations.characterId, characterId))
    .limit(1);
...
export function projectOnboardingFoundationForFinalReview(
  evidence: AcceptedOnboardingFoundationEvidence,
  foundation: CharacterFoundationRecord,
): OnboardingFoundationPublicReview {
  return {
    identity: {
