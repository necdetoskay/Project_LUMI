...
function readEvidence(summary: Record<string, unknown>): AcceptedOnboardingFoundationEvidence {
  const identity = summary.characterIdentity as AcceptedOnboardingFoundationEvidence["identity"] | undefined;
  const universe = summary.universe as AcceptedOnboardingFoundationEvidence["universe"] | undefined;
  const world = summary.world as AcceptedOnboardingFoundationEvidence["world"] | undefined;
  const compatibility = summary.compatibility as AcceptedOnboardingFoundationEvidence["compatibility"] | undefined;
...
export async function prepareCharacterFoundationCommit(
  ...
  }).from(parentalSettings).where(eq(parentalSettings.householdId, input.householdId)).limit(1);
...
export async function completeCharacterFoundationCommit(input: {
  ...
  const db = getProfileDb();
  const [cycle] = await db.select().from(characterCreationCycles).where(and(
  ...
  const summary = (cycle.latestSummary ?? {}) as Record<string, unknown>;
  if (summary.committedCharacterId !== input.characterId) throw new Error("FOUNDATION_CHARACTER_MISMATCH");
