...
describe("S06 - Trait Vector Validation", () => {
  it("accepts valid trait vector with all dimensions", () => {
    expect(() =>
      validateTraitVector(DEFAULT_CHILD_AVATAR_TRAITS),
    ).not.toThrow();
  });

  it("rejects unknown trait dimension", () => {
    expect(() =>
      validateTraitVector({ courage: 0.5, unknown_trait: 0.3 } as never),
    ).toThrow(ValidationError);
  });

  it("rejects NaN trait value", () => {
    expect(() => validateTraitVector({ courage: NaN })).toThrow(
      ValidationError,
    );
  });

  it("rejects out-of-range trait value (negative)", () => {
    expect(() => validateTraitVector({ courage: -0.1 })).toThrow(
      ValidationError,
    );
  });

  it("rejects out-of-range trait value (>1)", () => {
    expect(() => validateTraitVector({ courage: 1.5 })).toThrow(
      ValidationError,
    );
  });
...
describe("S06 - Emotion Vector Validation", () => {
  it("accepts valid emotion vector", () => {
    expect(() =>
      validateEmotionVector(DEFAULT_CHILD_AVATAR_EMOTIONS),
    ).not.toThrow();
  });

  it("rejects unknown emotion dimension", () => {
    expect(() =>
      validateEmotionVector({ joy: 0.5, unknown: 0.3 } as never),
    ).toThrow(ValidationError);
  });
...
describe("S06 - Needs Validation", () => {
  it("accepts valid needs array", () => {
    expect(() =>
      validateNeeds([{ needType: "hunger", value: 0.5, decay: 0.05 }]),
    ).not.toThrow();
  });
  ...
  it("rejects unknown need type", () => {
    expect(() =>
      validateNeeds([
        { needType: "unknown" as never, value: 0.5, decay: 0.05 },
      ]),
    ).toThrow(ValidationError);
  });
  ...
  it("rejects need value out of range", () => {
    expect(() =>
      validateNeeds([{ needType: "hunger", value: -0.1, decay: 0.05 }]),
    ).toThrow(ValidationError);
  });

  it("rejects NaN need value", () => {
    expect(() =>
      validateNeeds([{ needType: "hunger", value: NaN, decay: 0.05 }]),
    ).toThrow(ValidationError);
  });
});
...
describe("S06 - LumiCharacter: Trait Delta with Evidence", () => {
  it("applies bounded trait delta with evidence", () => {
    ...
    c.applyTraitDelta({
      dimension: "courage",
      oldValue: oldCourage,
      newValue: Math.min(1, oldCourage + 0.1),
      evidence: "Character helped a friend in need",
      deltaMagnitude: 0.1,
    });
...
describe("S06 - LumiCharacter: Forged oldValue / Server-Authoritative Delta", () => {
  it("rejects forged oldValue when current state value differs (courage 0.5 → forged 0.85 → 1.0)", () => {
    const c = makeCharacter();
    expect(c.getState().traits.courage).toBe(
      DEFAULT_CHILD_AVATAR_TRAITS.courage,
    );
    expect(() =>
      c.applyTraitDelta({
        dimension: "courage",
  ...
  it("applyTraitDeltas returns server-computed ResolvedTraitDelta[] on success", () => {
    ...
    const resolved = c.applyTraitDeltas([
      ...
      {
        dimension: "curiosity",
        oldValue: currentCuriosity,
        newValue: currentCuriosity + 0.05,
        evidence: "b2",
      },
    ]);
  ...
  it("rejects duplicate trait dimension in same batch with DUPLICATE_TRAIT_DELTA_DIMENSION", () => {
    ...
    expect(() =>
      c.applyTraitDeltas([
        ...
        { dimension: "courage", newValue: 0.6, evidence: "second" },
      ]),
    ).toThrow(ValidationError);
  });
  ...
  it("duplicate dimension rejection preserves atomicity: trait state and version unchanged", () => {
    ...
    expect(() =>
      c.applyTraitDeltas([
        ...
        { dimension: "courage", newValue: 0.6, evidence: "duplicate" },
      ]),
    ).toThrow(ValidationError);
    expect(c.getState().traits.courage).toBe(originalCourage);
    expect(c.getState().traits.curiosity).toBe(originalCuriosity);
  ...
  it("duplicate dimension rejection includes correct error code", () => {
    ...
    try {
      c.applyTraitDeltas([
        ...
        { dimension: "courage", newValue: 0.6, evidence: "b" },
      ]);
      throw new Error("should have thrown");
    } catch (err) {
...
describe("S06 - Lifecycle Stage Validation", () => {
  ...
  it("rejects invalid stage", () => {
    expect(() => validateCharacterLifecycleStage("unknown")).toThrow(
      ValidationError,
    );
  });
});
