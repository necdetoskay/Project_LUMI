...
beforeAll(async () => {
  ...
  try {
    ...
    console.warn(
      "Character domain integration database unavailable - skipping tests",
    );
    console.warn(error);
  }
});
...
describe("S06 - Character Domain Repository Integration (DB-gated)", () => {
  ...
  itIfDb("influence persistence", async () => {
    ...
    await repo.upsertInfluence(character.id, {
      emotional: 0.5,
      social: 0.3,
      cultural: 0.1,
      educational: 0.2,
...
describe("S06 - Transaction Rollback [repo-level] (DB-gated)", () => {
  itIfDb(
    "[repo-level] rolls back domain writes on version conflict",
    async () => {
      const { character, db: d } = await setupCharacter();
      ...
      await d.transaction(async (tx) => {
        ...
        await txDomainRepo.upsertTraitState({
          characterId: character.id,
          dimension: "courage",
          value: 0.8,
        });
      ...
      const finalRecord = await charRepo.findById(
        character.id,
  ...
  itIfDb(
    "[repo-level] rolls back event write on version conflict",
    async () => {
      const { character, db: d } = await setupCharacter();
      const domainRepo = getDomainRepo(d as never);
      // First mutation succeeds
...
describe("S06 - Optimistic Version Conflict [repo-level] (DB-gated)", () => {
  itIfDb(
    "[repo-level] two concurrent mutations - first succeeds, second gets VERSION_CONFLICT",
    async () => {
      const { character, db: d } = await setupCharacter();
      const charRepo = new DrizzleCharacterRepository(d as never);
      ...
      // Verify version is 2
      const record = await charRepo.findById(
        character.id,
        character.householdId,
      );
...
describe("S06 - Service-Level Mutation Audit (DB-gated)", () => {
  ...
  itIfDb(
    "uses test DB for both getRepos() reads and resolveDb() writes after __setTestDb injection",
    async () => {
      const { character, db: d } = await setupCharacter();

      const domainRepo = new DrizzleCharacterDomainRepository(d as never);
      await domainRepo.upsertTraitState({
        characterId: character.id,
        dimension: "courage",
        value: 0.7,
      });
      ...
      const result = await getCharacterDomain(
        TEST_USER_ID,
        character.householdId,
        character.id,
      );
  ...
  itIfDb(
    "applyTraitDeltas (single delta) produces event + trait state + history + version=2",
    async () => {
      ...
      const dbRecord = await charRepo.findById(
        character.id,
        character.householdId,
      );
  ...
  itIfDb(
    "applyTraitDeltas (multi-delta) version consistency: response.version === DB.version === event.aggregateVersion",
    async () => {
      const { character } = await setupCharacter();
  ...
  itIfDb(
    "updateEmotions produces CHARACTER_EMOTION_UPDATED event with version=2",
    async () => {
      const { character } = await setupCharacter();
      const emotions = {
        joy: 0.9,
      ...
      const result = await updateEmotions(
        TEST_USER_ID,
        character.householdId,
        character.id,
        emotions,
  ...
  itIfDb(
    "updateNeeds produces CHARACTER_NEEDS_UPDATED event with version=2",
    async () => {
      const { character } = await setupCharacter();
  ...
  itIfDb(
    "addGoal + completeGoal produce events with correct versions",
    async () => {
      const { character } = await setupCharacter();

      const addResult = await addGoal(
        TEST_USER_ID,
        character.householdId,
        character.id,
        {
      ...
      const completeResult = await completeGoal(
        TEST_USER_ID,
        character.householdId,
        character.id,
        goals[0]!.id,
      ...
      const addEvent = events.find(
        (e) => e.eventType === "CHARACTER_GOAL_ADDED",
      );
      const completeEvent = events.find(
        (e) => e.eventType === "CHARACTER_GOAL_COMPLETED",
      );
  ...
  itIfDb(
    "upsertInfluence produces CHARACTER_INFLUENCE_UPDATED event with version=2",
    async () => {
      const { character } = await setupCharacter();

      const result = await upsertInfluence(
        TEST_USER_ID,
        character.householdId,
        character.id,
        {
  ...
  itIfDb(
    "addRelationship produces CHARACTER_RELATIONSHIP_ADDED event with version=2",
    async () => {
  ...
  itIfDb(
    "updateLocation produces CHARACTER_LOCATION_CHANGED event with version=2",
    async () => {
      const { character } = await setupCharacter();
  ...
  itIfDb(
    "all mutations record immutable event rows with correct version",
    async () => {
      const { character } = await setupCharacter();

      await applyTraitDeltas(
        TEST_USER_ID,
        character.householdId,
        character.id,
        [
...
describe("S06 - Trait Delta Bounded: Server-Authoritative oldValue (DB-gated)", () => {
  ...
  itIfDb(
    ...
    async () => {
      ...
      const charRepo = new DrizzleCharacterRepository(db! as never);
      const versionBefore = (
        await charRepo.findById(character.id, character.householdId)
      )?.version;
      ...
      const charAfter = await charRepo.findById(
        character.id,
        character.householdId,
      );
        ...
        await applyTraitDeltas(
          ...
          [
            { dimension: "courage", newValue: 0.55, evidence: "first courage" },
            {
              dimension: "curiosity",
              newValue: 0.65,
              evidence: "valid curiosity",
            },
            {
              dimension: "courage",
              newValue: 0.6,
              evidence: "duplicate courage",
            },
          ],
        );
        throw new Error("expected applyTraitDeltas to throw");
