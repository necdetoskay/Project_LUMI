# LUMI — Simulation Domain

## 1. Domainin Amacı

Dünya simülasyonu, zaman ilerlemesi, dünya olayları, arka plan eylemleri ve durum değişiklikleri.

Bu domain; **simulation run, world event, state change, background action, utility score** yaşam döngüsünün tek sahibidir. Başka domainler bu verilere doğrudan yazmak yerine ilgili repository veya application service üzerinden erişir.
...
- `simulation.simulation_runs` — Her simülasyon çalıştırması
- `simulation.world_events` — Dünya olayları
- `simulation.state_changes` — Entity durum değişikliği günlüğü
- `simulation.background_actions` — NPC ve dünya arka plan eylemleri
- `simulation.utility_scores` — Karar değerlendirme skorları
...
Bu domainin çekirdek tablolarında soft delete zorunlu değildir.
...
simulation_runs, state_changes, background_actions, utility_scores
...
```text
src/db/schema/simulation_domain/
├── index.ts
