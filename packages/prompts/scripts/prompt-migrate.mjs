...
async function getAppliedFiles(pool) {
  try {
    const result = await pool.query(
      "SELECT filename FROM prompts._prompt_migration_ledger ORDER BY id",
    );
...
async function main() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      await pool.query(
        "INSERT INTO prompts._prompt_migration_ledger (filename) VALUES ($1)",
        [file],
      );
      console.warn(`Migration ${file} applied successfully`);
    }
    console.warn(`All ${files.length} prompt migrations applied successfully`);
  } catch (error) {
