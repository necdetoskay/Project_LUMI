...
export const testLabBranches = aiSchema.table(
  ...
  {
    id: primaryId(),
    sessionId: uuid("session_id").notNull().references(() => testLabSessions.id, { onDelete: "cascade" }),
...
export const testLabStateSnapshots = aiSchema.table(
  "test_lab_state_snapshots",
  {
    id: primaryId(),
    sessionId: uuid("session_id").notNull().references(() => testLabSessions.id, { onDelete: "cascade" }),
    branchId: uuid("branch_id").notNull().references(() => testLabBranches.id, { onDelete: "cascade" }),
...
export const testLabRuns = aiSchema.table(
  "test_lab_runs",
  {
    ...
    parentStateId: uuid("parent_state_id").notNull().references(() => testLabStateSnapshots.id),
  ...
  (table) => [
    ...
    check("chk_test_lab_run_model_pricing_pair", sql`(${table.modelSlug} IS NULL AND ${table.pricingSnapshot} IS NULL) OR (${table.modelSlug} IS NOT NULL AND ${table.pricingSnapshot} IS NOT NULL)`),
    check("chk_test_lab_run_usage_traceable", sql`${table.usageSnapshot} IS NULL OR (${table.modelSlug} IS NOT NULL AND ${table.pricingSnapshot} IS NOT NULL)`),
  ],
);
...
export const testLabRunCandidates = aiSchema.table(
  ...
  {
    id: primaryId(),
    runId: uuid("run_id").notNull().references(() => testLabRuns.id, { onDelete: "cascade" }),
    sessionId: uuid("session_id").notNull().references(() => testLabSessions.id, { onDelete: "cascade" }),
    branchId: uuid("branch_id").notNull().references(() => testLabBranches.id, { onDelete: "cascade" }),
    phaseId: varchar("phase_id", { length: 160 }).notNull(),
    ordinal: integer("ordinal").notNull(),
    payload: jsonb("payload").notNull(),
    candidateStateId: uuid("candidate_state_id").notNull().references(() => testLabStateSnapshots.id),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "date" }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex("test_lab_candidate_state_uq").on(table.candidateStateId),
    uniqueIndex("test_lab_candidate_run_ordinal_uq").on(table.runId, table.ordinal),
...
export const testLabSelections = aiSchema.table(
  "test_lab_selections",
  {
    ...
    branchId: uuid("branch_id").notNull().references(() => testLabBranches.id, { onDelete: "cascade" }),
    phaseId: varchar("phase_id", { length: 160 }).notNull(),
    runId: uuid("run_id").notNull().references(() => testLabRuns.id),
    candidateId: uuid("candidate_id").notNull().references(() => testLabRunCandidates.id),
    selectedStateId: uuid("selected_state_id").notNull().references(() => testLabStateSnapshots.id),
    actor: varchar("actor", { length: 20 }).notNull(),
    strategy: varchar("strategy", { length: 80 }),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "date" }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex("test_lab_one_selection_per_branch_phase_uq").on(table.sessionId, table.branchId, table.phaseId),
...
export type TestLabRunCandidateRecord = typeof testLabRunCandidates.$inferSelect;
