...
async function getAppliedFiles(pool) {
  try {
    const result = await pool.query(
      "SELECT filename FROM ai._ai_migration_ledger ORDER BY id",
    );
...
async function main() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      await pool.query(
        "INSERT INTO ai._ai_migration_ledger (filename) VALUES ($1)",
        [file],
      );
      console.warn(`Migration ${file} applied successfully`);
    }
    console.warn(`All ${files.length} ai migrations applied successfully`);
  } catch (error) {
