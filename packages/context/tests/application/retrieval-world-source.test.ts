...
describe("RetrievalWorldEventSource", () => {
  it("maps retrieved world events into canonical semantic items", async () => {
    const retrieval: ContextRetrievalSource = { retrieve: async () => ({ candidates: [{ stableId: "event:storm", sourceKind: "world-event", summary: "A crystal storm is approaching the floating islands.", relevance: 0.92, provenance: { authority: "world-event-store", recordId: "event:storm" } }] }) };
