...
import { InMemoryEmotionalStateAdapter, InMemoryKnowledgeAdapter, InMemoryLongTermMemoryAdapter, InMemoryParentPolicyAdapter, InMemorySafetyPolicyAdapter, InMemoryWorkingStoryAdapter, InMemoryWorldAdapter } from "../../src/adapters";
import type { TokenBudget } from "../../src/ports";
import { testEmotionalState, testKnowledge, testLongTermMemories, testParentPolicy, testRequest, testSafetyPolicy, testWorkingStory, testWorld } from "../fixtures/contexts";

const budget: TokenBudget = { totalTokens: 400, safetyTokens: 80, parentPolicyTokens: 40, workingStoryTokens: 120, emotionalStateTokens: 40, longTermMemoryTokens: 40, knowledgeTokens: 40, worldTokens: 5 };
...
describe("ContextBuilder semantic world budget", () => {
  it("keeps critical location while truncating lower-value world details", async () => {
    ...
    expect(world?.items.map((item) => item.id)).not.toContain("world:environment");
    expect(world?.tokensUsed).toBeLessThanOrEqual(budget.worldTokens);
    expect(manifest.findings.some((finding) => finding.code === "SECTION_TRUNCATED" && finding.section === "world")).toBe(true);
  });
});
