import { check, index, jsonb, numeric, timestamp, uuid, varchar } from "drizzle-orm/pg-core";
...
export const npcBeliefs = npcIntelligenceSchema.table(
  ...
  (table) => [
    ...
    check("npc_beliefs_confidence_check", sql`${table.confidence} >= 0 AND ${table.confidence} <= 1`),
    check("npc_beliefs_status_check", sql`${table.status} IN ('active','stale','expired')`)
  ]
);
