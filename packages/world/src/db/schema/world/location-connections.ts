...
export const worldLocationConnections = profileSchema.table(
  "world_location_connections",
  {
    ...
    connectionType: varchar("connection_type", { length: 30 }).notNull().default("path"),
    traversalCost: integer("traversal_cost").notNull().default(1),
    isBidirectional: boolean("is_bidirectional").notNull().default(true),
    accessibilityRequirement: varchar("accessibility_requirement", { length: 20 }),
  ...
  (table) => [
    uniqueIndex("uq_location_connection").on(table.fromLocationId, table.toLocationId),
...
export type WorldLocationConnectionRecord = typeof worldLocationConnections.$inferSelect;
export type NewWorldLocationConnectionRecord = typeof worldLocationConnections.$inferInsert;
