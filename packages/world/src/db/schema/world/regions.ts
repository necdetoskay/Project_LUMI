...
export const worldRegions = profileSchema.table(
  "world_regions",
  {
    ...
    accessibilityStatus: varchar("accessibility_status", { length: 20 }).notNull().default("open"),
    discoveryStatus: varchar("discovery_status", { length: 20 }).notNull().default("unknown"),
  ...
  (table) => [
    ...
    check("wr_accessibility_check", sql`${table.accessibilityStatus} IN ('open', 'restricted', 'blocked', 'dangerous')`),
    check("wr_discovery_check", sql`${table.discoveryStatus} IN ('unknown', 'rumored', 'discovered', 'explored')`),
    check("wr_region_type_check", sql`${table.regionType} IN ('wilderness', 'settlement', 'water', 'mountain', 'forest', 'sky', 'underground', 'magical', 'urban', 'coastal', 'island', 'custom')`),
  ],
);
