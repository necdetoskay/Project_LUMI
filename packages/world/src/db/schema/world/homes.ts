...
export const worldHomes = profileSchema.table(
  "world_homes",
  {
    ...
    homeType: varchar("home_type", { length: 20 })
      .notNull()
      .default("permanent"),
  ...
  (table) => [
    ...
    check(
      "wh_home_type_check",
      sql`${table.homeType} IN ('permanent', 'temporary', 'safe_haven')`,
    ),
    check(
      "wh_residence_type_check",
      sql`${table.residenceType} IN ('primary', 'secondary', 'guest')`,
    ),
  ],
);
