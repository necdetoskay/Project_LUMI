...
async function ensureLedger(pool) {
  await pool.query("CREATE SCHEMA IF NOT EXISTS privacy;");
  await pool.query(`
    CREATE TABLE IF NOT EXISTS privacy._privacy_migration_ledger (
...
async function getAppliedFiles(pool) {
  try {
    const result = await pool.query(
      "SELECT filename FROM privacy._privacy_migration_ledger ORDER BY id",
    );
...
async function main() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      await pool.query(
        "INSERT INTO privacy._privacy_migration_ledger (filename) VALUES ($1)",
        [file],
      );
      console.warn(`Migration ${file} applied successfully`);
    }
    console.warn(`All ${files.length} privacy migrations applied successfully`);
  } catch (error) {
