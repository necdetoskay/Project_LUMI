...
export class DrizzleStoryRepository implements StoryRepository {
  ...
  async findDefinitionById(
    ...
    const [row] = await tx
      ...
      .where(eq(storyDefinitions.id, id))
      .limit(1);
    return row;
  }

  async findDefinitionsByHousehold(
    tx: { select: QueryExecutor["select"] },
    householdId: string,
  ) {
    return tx
  ...
  async findVersionById(tx: { select: QueryExecutor["select"] }, id: string) {
    const [row] = await tx
      .select()
      .from(storyVersions)
      .where(eq(storyVersions.id, id))
      .limit(1);
    return row;
  }

  async findVersionsByDefinition(
    tx: { select: QueryExecutor["select"] },
  ...
  async createTransition(
    ...
    const [row] = await tx
      .insert(storySceneTransitions)
      .values(data)
      .returning();
    return row!;
  }

  async findTransitionsByVersion(
    tx: { select: QueryExecutor["select"] },
    storyVersionId: string,
  ) {
    return tx
      .select()
      .from(storySceneTransitions)
      .where(eq(storySceneTransitions.storyVersionId, storyVersionId));
  }

  async createChoicePoint(
    tx: { insert: QueryExecutor["insert"] },
  ...
  async findChoicePointById(
    ...
    const [row] = await tx
      ...
      .where(eq(storyChoicePoints.id, id))
      .limit(1);
    return row;
  }

  async findChoicePointsByScene(
    tx: { select: QueryExecutor["select"] },
    sceneId: string,
  ) {
    return tx
  ...
  async findChoiceOptionById(
    ...
    const [row] = await tx
      ...
      .where(eq(storyChoiceOptions.id, id))
      .limit(1);
    return row;
  }

  async findChoiceOptionsByPoint(
    tx: { select: QueryExecutor["select"] },
  ...
  async createCommittedChoice(
    ...
    const [row] = await tx
      .insert(storyCommittedChoices)
      .values(data)
      .returning();
    return row!;
  }

  async findCommittedChoiceByPoint(
    tx: { select: QueryExecutor["select"] },
  ...
  async findCommittedChoicesBySession(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    return tx
  ...
  async createChoiceConsequence(
    ...
    const [row] = await tx
      .insert(storyChoiceConsequences)
      .values(data)
      .returning();
    return row!;
  }

  async findConsequencesBySession(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    return tx
  ...
  async createOutcomeCandidate(
    ...
    const [row] = await tx
      .insert(storyOutcomeCandidates)
      .values(data)
      .returning();
    return row!;
  }

  async findLatestOutcomeCandidateBySession(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    const [row] = await tx
      .select()
      .from(storyOutcomeCandidates)
    ...
    const where =
      expectedVersion !== undefined
        ? and(
            ...
            eq(storySessions.version, expectedVersion),
          )
        : eq(storySessions.id, id);
    const [row] = await tx
  ...
  async createSessionCharacter(
    tx: { insert: QueryExecutor["insert"] },
    data: NewStorySessionCharacterRecord,
  ) {
    const [row] = await tx
      .insert(storySessionCharacters)
      .values(data)
      .returning();
    return row!;
  }

  async findSessionCharacters(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    return tx
      .select()
      .from(storySessionCharacters)
      .where(eq(storySessionCharacters.storySessionId, storySessionId));
  }

  async createSceneVisit(
    tx: { insert: QueryExecutor["insert"] },
    data: NewStorySessionSceneVisitRecord,
  ) {
    const [row] = await tx
      .insert(storySessionSceneVisits)
      .values(data)
      .returning();
    return row!;
  }

  async findSceneVisitsBySession(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    return tx
      .select()
      .from(storySessionSceneVisits)
  ...
  async createCheckpoint(
    ...
    const [row] = await tx
      .insert(storySessionCheckpoints)
      .values(data)
      .returning();
    return row!;
  }

  async findCheckpointsBySession(
    tx: { select: QueryExecutor["select"] },
    storySessionId: string,
  ) {
    return tx
      .select()
      .from(storySessionCheckpoints)
