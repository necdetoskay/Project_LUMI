...
export const storyOutcomeCandidates = storySchema.table(
  "story_outcome_candidates",
  {
    ...
    candidateSchemaVersion: integer("candidate_schema_version").notNull().default(1),
  ...
  (table) => [
    ...
    index("story_outcome_candidate_consequence_idx").on(table.sourceConsequenceId),
    check(
      "story_outcome_candidate_status_check",
      sql`${table.status} IN ('pending', 'committed', 'rejected', 'superseded')`,
    ),
    check("story_outcome_candidate_schema_version_positive_check", sql`${table.candidateSchemaVersion} >= 1`),
  ],
);

export type StoryOutcomeCandidateRecord = typeof storyOutcomeCandidates.$inferSelect;
export type NewStoryOutcomeCandidateRecord = typeof storyOutcomeCandidates.$inferInsert;
