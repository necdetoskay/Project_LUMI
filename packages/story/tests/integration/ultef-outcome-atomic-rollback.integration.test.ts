...
async function readState(
  ...
  const visits = await database
    ...
    .where(
      eq(schema.storySessionSceneVisits.storySessionId, fixture.storySessionId),
    );
  const checkpoints = await database
    .select()
    .from(schema.storySessionCheckpoints)
    .where(
      eq(schema.storySessionCheckpoints.storySessionId, fixture.storySessionId),
    );
...
describeDb("ULTEF Sprint 01 — atomic outcome rollback", () => {
  ...
  it("L4-OUTCOME-ROLLBACK-001 rolls back session advance when outcome validation fails", async () => {
