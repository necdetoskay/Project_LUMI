...
function requireSafeTestDb(): string {
  ...
  if (
    !name ||
    SAFE_DENYLIST.includes(name) ||
    (!name.includes("test") && !name.includes("review"))
  ) {
    throw new Error(
      `ULTEF BLOCKED: unsafe destructive database name '${name}'`,
    );
  }
  return DATABASE_URL;
}
...
describe.skipIf(!ENABLE_DESTRUCTIVE)(
  "PX-LUMI-09-001 outcome commit narrative",
  () => {
    it("persists an NPC story outcome, world-version delta, event evidence and idempotency", async () => {
      ...
      try {
        ...
        scenario.setup("Observed state before story outcome", {
          "need.hunger": 40,
        });
        ...
        scenario.delta(
          "world.version",
          first.worldVersionBefore,
          first.worldVersionAfter,
          "story outcome commit",
        );
        scenario.delta(
          "proposedChange.Mira.need.hunger",
          40,
          80,
          "validated world change recorded by commit pipeline",
        );
        ...
        const persistedEvents = await db
          ...
          .where(
            eq(schema.storyEventStore.storySessionId, fixture.storySessionId),
          );
        ...
        scenario.event(
          "world.commit.reloaded",
          "Commit, world version, event ve indirect-effect outbox kayıtları veritabanından yeniden okundu.",
        );
        ...
        scenario.event(
          "world.commit.retried",
          "Aynı manifest ikinci kez uygulandı; idempotency guard mevcut commit'i yeniden kullandı.",
          {
        ...
        scenario.assert(
          ...
          1,
          persistedCommits.length,
        );
        scenario.assert(
          "World version persisted",
          persistedVersion.length === 1,
          String(first.worldVersionAfter),
          persistedVersion[0]?.currentVersion,
        );
        scenario.assert(
          "World commit event persisted",
          persistedEvents.some(
            (event) => event.eventType === "STORY_WORLD_COMMIT_APPLIED",
          ),
          true,
          persistedEvents.map((event) => event.eventType),
        );
        scenario.assert(
          "Indirect NPC rumor intent persisted",
          persistedOutbox.some((row) => row.intentType === "npc_rumor_spread"),
          true,
          persistedOutbox.map((row) => row.intentType),
        );
        scenario.assert(
          "Retry reused same commit",
          second.commitId === first.commitId,
          first.commitId,
          second.commitId,
        );
        scenario.assert(
          "Retry did not duplicate commit",
          commitsAfterRetry.length === 1,
          1,
          commitsAfterRetry.length,
        );
        ...
        const passed =
          ...
          persistedOutbox.some(
            (row) => row.intentType === "npc_rumor_spread",
          ) &&
          second.commitId === first.commitId &&
          commitsAfterRetry.length === 1;
