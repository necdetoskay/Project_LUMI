...
function checkFile(filePath, fname) {
  ...
  try {
    ...
    console.log(
      `${rel}:1 [NOT_UTF8] ${e.message} (${raw.length} bytes, hex: ${hex})`,
    );
    foundAny = true;
    return;
  }
...
function decodeUTF8(buf) {
  ...
  while (i < len) {
    ...
    if (b < 0x80) {
      ...
      if (i + 1 >= len)
        throw new Error(`Truncated 2-byte sequence at offset ${i}`);
      const b2 = buf[i + 1];
      if (b2 < 0x80 || b2 > 0xbf)
      ...
      if (i + 2 >= len)
        throw new Error(`Truncated 3-byte sequence at offset ${i}`);
      const b2 = buf[i + 1];
      const b3 = buf[i + 2];
      ...
      if (i + 3 >= len)
        throw new Error(`Truncated 4-byte sequence at offset ${i}`);
      const b2 = buf[i + 1];
      const b3 = buf[i + 2];
      const b4 = buf[i + 3];
      if (
      ...
      throw new Error(
        `Invalid UTF-8 leading byte 0x${b.toString(16)} at offset ${i}`,
      );
    }
  }
