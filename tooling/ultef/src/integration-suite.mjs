...
const counts = Object.fromEntries(['PASS', 'WARN', 'FAIL', 'BLOCKED'].map((key) => [key, results.filter((r) => r.result === key).length]));
const overall = counts.FAIL > 0 ? 'FAIL' : counts.BLOCKED > 0 ? 'BLOCKED' : counts.WARN > 0 ? 'WARN' : 'PASS';
...
await writeFile(path.join(latest, 'integration-summary.json'), `${JSON.stringify(suite, null, 2)}\n`, 'utf8');
await writeFile(path.join(latest, 'integration-summary.md'), renderSuite(suite), 'utf8');
await writeFile(
...
function renderSuite(suite) {
  ...
  lines.push('', 'A scenario without runtime evidence is never counted as PASS.');
  return `${lines.join('\n')}\n`;
}
