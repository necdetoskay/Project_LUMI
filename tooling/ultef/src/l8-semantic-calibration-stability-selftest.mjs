...
function calibration({ mae, bias, eligible = true, choice = mae, personality = mae, age = mae }) {
  return {
    eligible,
    mae,
    meanBias: bias,
...
const stable = evaluateSemanticCalibrationStability([
  calibration({ mae: 0.63, bias: 0.63, choice: 0.83, personality: 0.5, age: 0.58 }),
  calibration({ mae: 0.65, bias: 0.65, choice: 0.84, personality: 0.5, age: 0.61 }),
  calibration({ mae: 0.64, bias: 0.64, choice: 0.82, personality: 0.51, age: 0.59 }),
]);
...
if (unstable.stable) throw new Error("Expected high-variance calibration set to fail.");
