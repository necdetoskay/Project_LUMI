...
export function createScenario({
  ...
  if (!id || !title || !level)
    throw new Error("ULTEF scenario requires id, title and level");
  ...
  return {
    ...
    finish({ result, reason = null, blockedBy = null }) {
      if (!ULTEF_RESULTS.includes(result))
        throw new Error(`Invalid ULTEF result: ${result}`);
...
export function renderNarrative(report) {
  ...
  lines.push("", "## Setup");
  for (const item of report.setup)
    lines.push(`- ${item.label}: ${format(item.value)}`);

  lines.push("", "## What happened");
  for (const item of report.timeline)
    lines.push(`${String(item.sequence).padStart(2, "0")}. ${item.summary}`);
  ...
  for (const item of report.stateDeltas) {
    lines.push(
      `- ${item.path}: ${format(item.before)} -> ${format(item.after)}${item.reason ? ` (${item.reason})` : ""}`,
    );
  }
