...
export async function writeScenarioArtifacts(report, options = {}) {
  ...
  await writeFile(path.join(runDir, `${safeName(report.id)}.json`), `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
  await writeFile(path.join(runDir, `${safeName(report.id)}.md`), narrative, 'utf8');

  await rm(latestDir, { recursive: true, force: true });
  await mkdir(latestDir, { recursive: true });
  await writeFile(path.join(latestDir, 'summary.json'), `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
  await writeFile(path.join(latestDir, 'summary.md'), narrative, 'utf8');
  if (report.result === 'FAIL' || report.result === 'BLOCKED') {
    await writeFile(path.join(latestDir, 'failures.json'), `${JSON.stringify({ id: report.id, result: report.result, reason: report.reason, blockedBy: report.blockedBy }, null, 2)}\n`, 'utf8');
  } else {
    await writeFile(path.join(latestDir, 'failures.json'), `${JSON.stringify({ failures: [] }, null, 2)}\n`, 'utf8');
  }
