...
const dataset = JSON.parse(
  await readFile(
    new URL("../calibration/l8-semantic-calibration-hard-boundary.json", import.meta.url),
    "utf8",
  ),
);
...
assert.match(markdown, /Arin, Mira'nın daha önce anlattığı köprü işaretini hatırladı/);
assert.match(markdown, /\| 4 \| 5 \|  \| pending \|/);
assert.match(markdown, /Mean signed bias: \*\*1\*\*/);
assert.match(markdown, /under=0, exact=0, over=18/);
assert.equal((markdown.match(/\| `(?:choice|personality|age)-boundary-/g) ?? []).length, 18);
