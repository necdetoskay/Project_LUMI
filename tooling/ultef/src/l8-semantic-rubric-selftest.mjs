...
const prompt = buildSemanticJudgePrompt({
  narratives: {
    choice:
      "Arin, daha önce Mira'ya sormayı seçtiği için onunla konuşmayı sürdürdü.",
    personality:
...
const parsed = parseSemanticJudgeResponse(
  JSON.stringify({
    scores: {
      choice_influence: {
        score: 5,
        reason: "The prior choice actively drives the scene.",
      },
      personality_emotion: {
        score: 4,
        reason: "Bora remains supportive and calm.",
      },
      age_appropriateness: {
        score: 5,
        reason: "The language is concrete and child-friendly.",
      },
    },
  }),
);
...
assert.throws(() => parseSemanticJudgeResponse("not-json"), /strict JSON/);
