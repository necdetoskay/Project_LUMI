...
scenario.event('npc.encountered', 'Arin, Mira ile karsilasti.', { npc: 'Mira' });
scenario.event('rumor.heard', 'Mira bir soylenti aktardi.', { rumorId: 'R-GOLDEN-001' });
scenario.delta('relationship.Arin.Mira.trust', 0.4, 0.46, 'conversation outcome');
scenario.assert('Rumor was recorded', true, 'R-GOLDEN-001', 'R-GOLDEN-001');
scenario.assert('Trust changed', true, 0.46, 0.46);

const report = scenario.finish({ result: 'PASS', reason: 'All self-test assertions passed.' });
...
try {
  const invalid = createScenario({ id: 'SELF-FAIL', title: 'False pass guard', level: 'L1' });
