# LUMI — Inventory Domain

## 1. Domainin Amacı

Öğe tanımları, gerçek öğe örnekleri, envanterler ve sahiplik hareketleri.

Bu domain; **item definition, item instance, inventory, inventory entry** yaşam döngüsünün tek sahibidir. Başka domainler bu verilere doğrudan yazmak yerine ilgili repository veya application service üzerinden erişir.
...
- `inventory.item_definitions` — Öğe türü tanımları
- `inventory.item_instances` — Dünyadaki gerçek öğe örnekleri
- `inventory.inventories` — Karakter veya profile ait envanter
- `inventory.inventory_entries` — Envanter içeriği
- `inventory.item_history` — Öğe sahiplik ve durum geçmişi
...
item_definitions, item_instances
...
```text
src/db/schema/inventory_domain/
├── index.ts
