# LUMI — Character Domain

## 1. Domainin Amacı

Çocuk avatarları, NPC’ler, özellikler, duygular, ilişkiler, hedefler ve koşullar.

Bu domain; **character, trait, emotion, relationship, goal, condition** yaşam döngüsünün tek sahibidir. Başka domainler bu verilere doğrudan yazmak yerine ilgili repository veya application service üzerinden erişir.
...
- `character.characters` — Tüm karakterlerin ana kaydı
- `character.trait_definitions` — Özellik tanımları
- `character.character_traits` — Karakter özellik vektörleri
- `character.emotion_definitions` — Duygu tanımları
- `character.character_emotions` — Anlık duygu vektörleri
- `character.relationships` — Yönlü karakter ilişkileri
- `character.relationship_dimensions` — İlişki boyutları
- `character.goals` — Karakter hedefleri
- `character.conditions` — Yaralanma, yorgunluk gibi koşullar
...
trait_history, relationship_events
...
```text
src/db/schema/character_domain/
├── index.ts
