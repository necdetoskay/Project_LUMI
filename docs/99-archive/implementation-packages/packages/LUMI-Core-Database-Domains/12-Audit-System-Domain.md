# LUMI — Audit & System Domain

## 1. Domainin Amacı

Denetim kayıtları, güvenlik olayları, outbox, idempotency ve feature flag altyapısı.

Bu domain; **audit log, security event, outbox event, idempotency key, feature flag** yaşam döngüsünün tek sahibidir. Başka domainler bu verilere doğrudan yazmak yerine ilgili repository veya application service üzerinden erişir.
...
- `audit.audit_logs` — İşlemsel denetim kayıtları
- `audit.security_events` — Güvenlik olayları
- `system.outbox_events` — Güvenilir event yayını
- `system.idempotency_keys` — Tekrarlı istek koruması
- `system.feature_flags` — Özellik bayrakları
...
audit.audit_logs, audit.security_events, system.outbox_events
...
```text
src/db/schema/audit_system_domain/
├── index.ts
...
| Konu | Karar |
|---|---|
| PostgreSQL schema | `audit / system` |
