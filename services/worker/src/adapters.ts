...
export class SimulationRepositoryWorldSourceAdapter implements WorldSourcePort {
  ...
  async fetchNpcsForWorld(): Promise<NpcSnapshot[]> { return []; }
  async fetchChildLastSeen(): Promise<Date | null> { return null; }
  async fetchScheduledEvents(worldId: string, householdId: string, unresolvedOnly: boolean): Promise<SimulationScheduledEvent[]> {
    return this.repo.findScheduledEvents(worldId, householdId, unresolvedOnly) as Promise<SimulationScheduledEvent[]>;
  }
  async updateClock(state: WorldClockState): Promise<void> { await this.repo.upsertClock(state); }
  async recordWorldEvent(worldId: string, eventType: string, payload: Record<string, unknown>): Promise<void> {
...
export class RepositoryNpcSourceAdapter implements NpcSourcePort {
  ...
  async fetchSnapshots(worldId: string, householdId: string): Promise<NpcSnapshot[]> {
    const rows = await this.snapshots.listForWorker(householdId, worldId, this.limit);
    return rows.map((row) => ({
      npcId: row.npcId,
...
export class EmptyRelevanceSourceAdapter implements RelevanceSourcePort {
  async fetchRelevanceBubble(worldId: string, householdId: string): Promise<RelevanceBubble> {
    return { worldId, householdId, entities: [], threshold: 0.25 };
  }
  async fetchEntityRelevance(): Promise<EntityRelevance | null> { return null; }
}
