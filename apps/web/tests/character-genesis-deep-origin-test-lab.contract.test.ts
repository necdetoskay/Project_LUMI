...
describe("Character Genesis Deep Origin Test Lab contract", () => {
  it(
    "mounts an isolated Deep Origin workbench without replacing onboarding Test Lab",
    () => {
  ...
  it(
    "persists prompt, raw output, parsed candidates, validation, tokens and cost evidence",
    () => {
      const panel = readApp("settings/test-lab/deep-origin-test-panel.tsx");
      const route = readApp("../api/settings/test-lab/genesis/origin/route.ts");
  ...
  it(
    "never commits deep-origin generation directly to the canonical universe",
    () => {
      const route = readApp("../api/settings/test-lab/genesis/origin/route.ts");
