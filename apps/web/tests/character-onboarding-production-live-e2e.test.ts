...
live("Character Onboarding 9-stage production Live E2E", () => {
  ...
  beforeAll(() => {
    const databaseName = new URL(url!).pathname
      .replace(/^\//, "")
      .toLowerCase();
  ...
  it("uses Context Builder + Prompt Registry + real OpenRouter across all 9 stages and commits the canonical foundation", async () => {
    ...
    try {
      ...
      const identities = await generateCharacterFirstIdentitySuggestions(
        userId,
        {
          householdId,
          childProfileId,
      ...
      const compatibilityResult = await generateCompatibilitySuggestions(
        userId,
        {
          householdId,
          childProfileId,
      ...
      expect(
        (
          await pool.query(`SELECT 1 FROM profile.world_homes WHERE id=$1`, [
      ...
      for (const step of canonicalSteps)
        expect(selectionRows.rows.some((row) => row.step_key === step)).toBe(
          true,
        );
      ...
      expect(traces.rows.every((row) => row.provider === "openrouter")).toBe(
        true,
      );
      expect(
      ...
      const traceJson = JSON.stringify(
        traces.rows.map((row) => row.input_context),
      );
      ...
      if (characterId) {
        await pool.query(
          `DELETE FROM profile.character_goals WHERE character_id=$1`,
          [characterId],
        );
