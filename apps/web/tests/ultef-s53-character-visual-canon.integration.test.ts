...
class FakeVisualProvider implements CharacterVisualGenerationPort {
  ...
  async generate(request: Parameters<CharacterVisualGenerationPort["generate"]>[0]) {
    this.calls += 1;
    return {
      provider: "fake-s53",
      model: request.model,
...
describe("PX-LUMI-S53 character visual canon", () => {
  it("persists multiple candidates, replays idempotently and replaces canon with history", async () => {
    ...
    expect(assets.find((asset) => asset.id === firstAsset.id)?.lifecycleState).toBe("archived");
    expect(assets.find((asset) => asset.id === secondAsset.id)?.lifecycleState).toBe("canonical");
    ...
    expect(afterReject.find((asset) => asset.id === firstAsset.id)?.lifecycleState).toBe("rejected");

    const persistedCanon = await getCharacterVisualCanon(USER_ID, HOUSEHOLD_ID, CHARACTER_ID);
