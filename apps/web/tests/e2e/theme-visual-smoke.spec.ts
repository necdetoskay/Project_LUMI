...
function uniqueSlug(label: string): string {
  householdCounter++;
  return `e2e-theme-${label}-${Date.now()}-${householdCounter}`
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "-");
}
...
test.describe("theme visual smoke", () => {
  ...
  test("login page: submit button bg-primary, password toggle NOT bg-primary", async ({
    page,
  }) => {
    ...
    const toggleBg = await toggle.evaluate(
      (el) => getComputedStyle(el).backgroundColor,
    );
    expect(toggleBg).not.toBe(PRIMARY_RGB);
  });

  test("register page: submit button bg-primary text-on-primary", async ({
    page,
  }) => {
  ...
  test("profiles page: Turkish copy regression", async ({ page }) => {
    ...
    const cpRes = await request.post("/api/child-profiles", {
      data: {
        householdId: (await hhRes.json()).household.id,
        displayName: "Theme Child",
        ageBand: "6-8",
      },
    });
  ...
  test("forgot-password page: Turkish copy", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Şifremi unuttum" }),
    ).toBeVisible();
  });
  ...
  test("reset-password page: Turkish copy", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Parolayı yenile" }),
    ).toBeVisible();
  });

  test("profiles page: 'Profili Aç' CTA bg-primary text-on-primary", async ({
    page,
  }) => {
    ...
    expect(await cta.evaluate((el) => getComputedStyle(el).color)).toBe(
      ON_PRIMARY_RGB,
    );
  ...
  test("character onboarding: AI archetype card theme states", async ({
    page,
  }) => {
    ...
    const settingsRes = await request.put("/api/settings/llm", {
      data: {
        action: "upsert-key",
        householdId,
        apiKey: "sk-or-v1-theme-test-key",
      },
    });
    ...
    await expect(
      page.getByRole("heading", { name: "Karakter Başlangıç Akışı" }),
    ).toBeVisible({ timeout: 15000 });
    await expect(
      page.getByRole("heading", { name: "Karakter arketipi seç" }),
    ).toBeVisible();
    ...
    expect(
      await secondCard.evaluate((el) => getComputedStyle(el).backgroundColor),
    ).toBe("rgb(255, 255, 255)");

    await firstCard.click();
    const selectedBg = await firstCard.evaluate(
      (el) => getComputedStyle(el).backgroundColor,
    );
