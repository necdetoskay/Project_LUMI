...
async function createParentAndProfile(page: Page) {
  ...
  const settingsResponse = await page.request.put("/api/settings/llm", {
    data: {
      action: "upsert-key",
      householdId,
      apiKey: "sk-or-v1-playwright-test-key",
    },
  });
...
test.describe("AI character onboarding", () => {
  test("renders regenerated LLM archetypes, origin provenance, and no fallback on failure", async ({
    page,
    request,
  }) => {
    ...
    const archetypeRequests = state.requests.filter(
    ...
    await expect(page.getByTestId("generate-origin-packages")).toBeVisible({
      timeout: 15_000,
    });
    ...
    await expect(
      page.getByText("mock-origin-model-v1", { exact: false }).first(),
    ).toBeVisible();
    ...
    const failNext = await request.post(`${mockControlUrl}/__mock/fail-next`, {
      data: { count: 2 },
    });
    expect(failNext.status()).toBe(200);
    await page.getByTestId("generate-origin-packages").click();

    await expect(page.getByTestId("origin-generation-error")).toBeVisible({
      timeout: 20_000,
    });
    await expect(originCards).toHaveCount(0);
    await expect(page.getByText("harita perisi", { exact: false })).toHaveCount(
      0,
    );
  });
    ...
    const unknown = await page.request.post(
      "/api/character-bootstrap/generate-archetypes",
      {
        data: {
          householdId,
          childProfileId,
          preferenceHints: { nested: { unsafe: true } },
        },
      },
    );
