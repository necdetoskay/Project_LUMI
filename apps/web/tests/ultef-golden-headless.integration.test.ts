...
ultefDescribe("ULTEF L6-GOLDEN-001 — canonical headless continuity journey", () => {
  it("runs Deniz and Arin from story scene through world commit, rumor materialization, reload and a later session", async () => {
    ...
    scenario.setup("Household", { id: fixture.householdId, alias: "H-GOLDEN-001" });
    scenario.setup("Child", { id: fixture.childProfileId, name: "Deniz", ageBand: "6-8" });
    ...
    try {
      scenario.event("profile.ready", "Deniz profili ve Arin karakteri aynı test household'u içinde hazırlandı.");
      scenario.event("world.ready", "Gunes Vadisi ve başlangıç hikâye session'ı hazırlandı.");

      const beforePlayback = await getSessionPlaybackState(fixture.storySessionId);
      scenario.assert(
      ...
      const propagation = await runtime.propagate({ householdId: fixture.householdId });
      const beliefs = await beliefRepository.getBeliefs(targetNpcId, fixture.householdId);
      ...
      await pool.query("DELETE FROM npc_intelligence.beliefs WHERE household_id = $1", [fixture.householdId]);
      ...
      await pool.query("DELETE FROM story.story_sessions WHERE household_id = $1 AND id <> $2", [fixture.householdId, fixture.storySessionId]);
