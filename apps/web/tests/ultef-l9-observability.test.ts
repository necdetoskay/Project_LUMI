...
ultefDescribe(
  "ULTEF L9-OBSERVABILITY-001 — operational signal contract",
  () => {
    it("emits correlated request, error, readiness and invalid-correlation signals aligned with alert config", async () => {
      ...
      try {
        ...
        const regeneratedCorrelationId =
          invalidResponse.headers.get(CORRELATION_HEADER);
        ...
        const alertMetricsBackedByRuntime = requiredAlertMetrics.every(
          (metric) =>
            alertMetricNames.has(metric) && emittedMetricNames.has(metric),
        );
