...
vi.mock("@lumi/story/application", () => ({
  getActiveSessionForChildAndWorld: (...args: unknown[]) => mockGetActiveSessionForChildAndWorld(...args),
...
vi.mock("@/lib/auth/with-parent", () => ({
  withParent: (fn: (parent: { id: string }) => Promise<Response>) => fn({ id: "ultef-parent" }),
}));
...
ultefDescribe("ULTEF L4-OPPORTUNITY-HOOK-001 — production composition boundary", () => {
  it("accepts an NPC opportunity and carries it through profile/world/session resolution into a story hook", async () => {
    ...
    scenario.setup("Execution boundary", "real web route composition with test-double package adapters");
    scenario.setup("Household", { id: HOUSEHOLD_ID, name: "ULTEF Household" });
    ...
    mockGetOwnedHousehold.mockImplementation(async () => {
      scenario.event("profile.household.resolved", "Parent ownership resolved to ULTEF Household.");
      return { id: HOUSEHOLD_ID, name: "ULTEF Household" };
    });
    mockFindChildProfileForUser.mockImplementation(async () => {
      scenario.event("profile.child.resolved", "Child profile Deniz was resolved inside the household.");
      return { id: CHILD_ID, displayName: "Deniz", ageBand: "6-8" };
    });
    mockRespond.mockImplementation(async (_householdId: string, _opportunityId: string, response: string) => {
      scenario.event("npc.opportunity.responded", `Rumor opportunity was marked '${response}'.`);
    });
    mockListCharactersByChildProfile.mockImplementation(async () => {
      scenario.event("profile.character.resolved", "Primary character Arin was resolved for Deniz.");
      return [{ id: CHARACTER_ID, name: "Arin" }];
    });
    mockGetWorldForCharacter.mockImplementation(async () => {
      scenario.event("world.resolved", "Arin's world Gunes Vadisi was resolved.");
      return { id: WORLD_ID, name: "Gunes Vadisi" };
    });
    mockGetActiveSessionForChildAndWorld.mockImplementation(async () => {
      scenario.event("story.session.resolved", "Active story session was resolved for Deniz in Gunes Vadisi.");
    ...
    mockCreateHook.mockImplementation(async (input: Record<string, unknown>) => {
      scenario.event(
        "story.hook.created",
    ...
    scenario.event("request.started", "Deniz accepted Mira's rumor opportunity.");
    ...
    const hookInput = mockCreateHook.mock.calls[0]?.[0] as Record<string, unknown> | undefined;

    scenario.assert("Route returns success", response.status === 200, 200, response.status);
    scenario.assert("Accepted opportunity transition executed", mockRespond.mock.calls.length === 1, 1, mockRespond.mock.calls.length);
    scenario.assert("World resolution used Arin", mockGetWorldForCharacter.mock.calls[0]?.[0] === CHARACTER_ID, CHARACTER_ID, mockGetWorldForCharacter.mock.calls[0]?.[0] ?? null);
    scenario.assert("Active session resolution uses Deniz and Gunes Vadisi", mockGetActiveSessionForChildAndWorld.mock.calls[0]?.[0] === CHILD_ID && mockGetActiveSessionForChildAndWorld.mock.calls[0]?.[1] === WORLD_ID, [CHILD_ID, WORLD_ID], mockGetActiveSessionForChildAndWorld.mock.calls[0] ?? null);
    scenario.assert("StoryHook points to active story session", hookInput?.storySessionId === SESSION_ID, SESSION_ID, hookInput?.storySessionId ?? null);
    scenario.assert("StoryHook points to resolved world", hookInput?.worldId === WORLD_ID, WORLD_ID, hookInput?.worldId ?? null);
    scenario.assert("Rumor claim reaches StoryHook payload", (hookInput?.payload as Record<string, unknown> | undefined)?.claim === "Eski koprunun isiklari firtinadan once yaniyor.", "Eski koprunun isiklari firtinadan once yaniyor.", (hookInput?.payload as Record<string, unknown> | undefined)?.claim ?? null);
    scenario.assert("Response exposes created rumor hook", body.hook?.id === HOOK_ID && body.hook?.hookType === "rumor", { id: HOOK_ID, hookType: "rumor" }, body.hook ?? null);

    scenario.delta("opportunity.status", "proposed", "accepted", "parent accepted NPC interaction");
    scenario.delta("story.hooks.count", 0, 1, "accepted rumor created a pending StoryHook");
    scenario.delta("story.hook.payload.claim", null, "Eski koprunun isiklari firtinadan once yaniyor.", "NPC evidence propagated into story composition");

    const passed = response.status === 200 && body.hook?.id === HOOK_ID && hookInput?.storySessionId === SESSION_ID && hookInput?.worldId === WORLD_ID;
    ...
    await writeScenarioArtifacts(report, { environment: "integration-composition-test-doubles" });
