...
export class OpenRouterTextGenerationProvider implements TextGenerationProvider {
  ...
  async generate(request: TextGenerationRequest): Promise<TextGenerationResult> {
    ...
    const model = request.model || process.env.LUMI_TEXT_MODEL;
    if (!model) throw new Error("No text generation model configured");
    ...
    return {
      ...
      estimatedCostUsd: typeof payload.usage?.cost === "number" ? payload.usage.cost : null,
    };
  }
}
