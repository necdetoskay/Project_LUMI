...
export async function POST(request: Request) {
  ...
  const body = (await request.json()) as { promptKey?: string; version?: number; context?: Record<string, unknown> };
  if (!body.promptKey || !Number.isInteger(body.version) || !body.context || typeof body.context !== "object" || Array.isArray(body.context)) return NextResponse.json({ message: "Invalid run request" }, { status: 400 });
  ...
  const preview = previewPrompt({ systemTemplate: target.systemTemplate, userTemplate: target.userTemplate, allowedVariables: target.allowedVariables, requiredVariables: target.requiredVariables }, body.context);
  if (preview.missingRequiredVariables.length || preview.unknownTemplateVariables.length) return NextResponse.json({ message: "Prompt validation failed", validation: { missingRequiredVariables: preview.missingRequiredVariables, unknownTemplateVariables: preview.unknownTemplateVariables } }, { status: 422 });
