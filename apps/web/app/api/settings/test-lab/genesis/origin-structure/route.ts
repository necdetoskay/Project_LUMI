  ...
  return withParent(async (parent) => {
    ...
    try {
      ...
      if (session.scenarioKey !== CHARACTER_ONBOARDING_SCENARIO.key) {
        throw new Error(`TEST_LAB_ORIGIN_STRUCTURE_REQUIRES_CHARACTER_ONBOARDING:${session.scenarioKey}`);
      }
      ...
      if (!optionalString(origin.summary) || !Array.isArray(origin.facts)) {
        throw new Error("TEST_LAB_ORIGIN_STRUCTURE_REQUIRES_SELECTED_DEEP_ORIGIN");
      }

      const extraction = toJsonObject({
        summary: origin.summary,
        facts: origin.facts,
        summaryFactIds: Array.isArray(origin.summaryFactIds) ? origin.summaryFactIds : [],
        unresolvedQuestions: Array.isArray(origin.unresolvedQuestions) ? origin.unresolvedQuestions : [],
        storyHooks: Array.isArray(origin.storyHooks) ? origin.storyHooks : [],
        evidence: {
          factCount: origin.facts.length,
          unresolvedQuestionCount: Array.isArray(origin.unresolvedQuestions) ? origin.unresolvedQuestions.length : 0,
          storyHookCount: Array.isArray(origin.storyHooks) ? origin.storyHooks.length : 0,
      ...
      return NextResponse.json({ data: { phaseId: PHASE_ID, run: recorded.run, candidates: recorded.candidates, extraction } });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
      return NextResponse.json({ error: "TEST_LAB_ORIGIN_STRUCTURE_ERROR", message }, { status: message.includes("FORBIDDEN") ? 403 : 400 });
    }
  });
