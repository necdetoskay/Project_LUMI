    ...
    return withParent(async (parent) => {
      ...
      try {
        if (parsed.action === "upsert-key") {
          ...
          const settings = await upsertOpenRouterKey(parent.id, householdId, parsed.apiKey);
          return NextResponse.json({ data: settings });
        }
        ...
        return NextResponse.json(
          { error: "VALIDATION_ERROR", message: `Unknown action: ${parsed.action}` },
          { status: 400 },
        );
      } catch (error) {
        return handleServiceError(error);
...
function handleServiceError(error: unknown): NextResponse {
  ...
  if (err.name === "AuthorizationError" || message.includes("not a member") || message.includes("FORBIDDEN")) {
    return NextResponse.json({ error: "FORBIDDEN", message }, { status: 403 });
  }
  if (err.code === "ENCRYPTION_CONFIG_ERROR") {
    return NextResponse.json({
      error: "ENCRYPTION_CONFIG_ERROR",
      message: "Sistem yapılandırma hatası. Şifreleme anahtarı eksik — lütfen sistem yöneticinize başvurun.",
    }, { status: 500 });
  }
