...
export const POST = observeHandler(async (request: Request) => {
  return withParent(async (parent) => {
    try {
      ...
      const originPackages = await listOriginPackages(
        parent.id,
        household.id,
        ...
        return NextResponse.json(
          {
            ...
            message: "No accepted origin package found for this character",
          },
          { status: 400 },
        );
      }
      ...
      const universeSeed =
        (seeds.universeSeed as string) ?? `universe-${characterId.slice(0, 8)}`;
      ...
      const result = await createWorldFromOrigin({
        ...
        acceptedCandidateSeed:
          (seeds.candidateSeed as string) ?? acceptedPackage.id,
        ...
        originPackage: {
          ...
          originConcept:
            character.originConcept ?? "A character exploring their world",
          startingRegionArchetype:
            character.startingLocation ?? "Starting Area",
          ...
          firstMysterySeed:
            (seeds.firstMysterySeed as string) ?? "mystery_seed",
        },
