  ...
  (request: Request, ctx: { params: Promise<{ id: string }> }) => {
    return withParent(async (parent) => {
      ...
      try {
        ...
        const home = await repo.findHomeByWorldId(db, worldId);

        if (!home) {
          return NextResponse.json(
            { error: "NOT_FOUND", message: "Home not found for this world" },
            { status: 404 },
          );
        }
        ...
        return NextResponse.json(
          { error: "INTERNAL_ERROR", message: err.message ?? "Failed to get home" },
          { status: 500 },
        );
      }
    });
