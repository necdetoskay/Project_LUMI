...
import { getOwnedHousehold, findChildProfileForUser, getCharacterDomain } from "@lumi/profiles/application";
...
function handleStoryError(error: unknown) {
  ...
  if (err.name === "ValidationError") {
    const status = err.code === "VERSION_NOT_PUBLISHED" || err.code === "SESSION_ALREADY_EXISTS" ? 409 : 400;
...
export const POST = observeHandler(async (request: Request, { params }: { params: Promise<{ storyId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      const childProfile = await findChildProfileForUser(childProfileId, parent.id, householdId);
      if (!childProfile) {
        return NextResponse.json(
          { error: "FORBIDDEN", message: "Child profile not found or not accessible" },
          { status: 403 },
        );
      }
      ...
      const character = await getCharacterDomain(parent.id, householdId, characterId);
      if (character.childProfileId !== childProfileId) {
        return NextResponse.json(
          { error: "FORBIDDEN", message: "Character does not belong to the given child profile" },
          { status: 403 },
        );
      }
