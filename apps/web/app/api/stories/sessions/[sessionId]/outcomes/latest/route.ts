...
import { getLatestOutcomeCandidate, getStorySessionOrForbidden } from "@lumi/story/application";
...
export const GET = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      ...
      const candidate = await getLatestOutcomeCandidate(sessionId);
      return NextResponse.json({ candidate });
    } catch (error) {
      return handleStoryError(error, "Failed to load latest outcome candidate");
    }
  });
}, "/api/stories/sessions/{sessionId}/outcomes/latest");
