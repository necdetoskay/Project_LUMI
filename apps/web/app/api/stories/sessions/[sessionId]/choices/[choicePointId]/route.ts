...
import { evaluateChoicePointAvailability, getStorySessionOrForbidden } from "@lumi/story/application";
...
export const GET = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string; choicePointId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      ...
      return handleStoryError(error, "Failed to evaluate choice point availability");
    }
  });
