...
import { getChoiceHistory, getStorySessionOrForbidden } from "@lumi/story/application";
...
export const GET = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      ...
      const history = await getChoiceHistory(sessionId);
      return NextResponse.json({ history });
    } catch (error) {
      return handleStoryError(error, "Failed to load choice history");
    }
  });
}, "/api/stories/sessions/{sessionId}/choices/history");
