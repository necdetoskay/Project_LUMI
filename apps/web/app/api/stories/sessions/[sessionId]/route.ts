...
import { getSessionPlaybackState, getStorySessionOrForbidden } from "@lumi/story/application";
...
export const GET = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      ...
      const result = await getSessionPlaybackState(sessionId);
      return NextResponse.json(result);
    } catch (error) {
      return handleStoryError(error, "Failed to load session playback state");
    }
  });
