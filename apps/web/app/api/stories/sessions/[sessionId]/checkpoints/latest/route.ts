...
import { getLatestCheckpoint, getStorySessionOrForbidden } from "@lumi/story/application";
...
export const GET = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      ...
      return handleStoryError(error, "Failed to load latest checkpoint");
    }
  });
}, "/api/stories/sessions/{sessionId}/checkpoints/latest");
