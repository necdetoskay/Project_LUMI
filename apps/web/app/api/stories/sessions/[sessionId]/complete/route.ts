...
export const POST = observeHandler(async (request: Request, { params }: { params: Promise<{ sessionId: string }> }) => {
  return withParent(async (parent) => {
    ...
    try {
      const result = await completeSession({ sessionId, expectedVersion, idempotencyKey });
      return NextResponse.json(result);
    } catch (error) {
      return handleStoryError(error, "Failed to complete session");
    }
  });
}, "/api/stories/sessions/{sessionId}/complete");
