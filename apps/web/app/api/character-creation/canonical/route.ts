    ...
    try {
      switch (body.action) {
        case "start":
          ...
          return NextResponse.json({ cycle: await chooseCharacterCreationDirection(parent.id, { ...input, direction: "character_first" }) });
        case "select-character-type":
          if (!body.characterType) return validation("characterType is required");
          return NextResponse.json({ cycle: await chooseCanonicalCharacterType(parent.id, { ...input, characterType: body.characterType }) });
        ...
        case "select-identity":
          return NextResponse.json({ cycle: await chooseCharacterIdentity(parent.id, { ...input, suggestion: body.suggestion as never }) });
        ...
        case "select-world":
          return NextResponse.json({ cycle: await chooseWorldSuggestion(parent.id, { ...input, suggestion: body.suggestion as never }) });
        ...
        case "select-compatibility":
          return NextResponse.json({ cycle: await chooseCompatibility(parent.id, { ...input, suggestion: body.suggestion as never }) });
        case "generate-region":
          return NextResponse.json(await generateRegionSuggestions(parent.id, input));
        case "select-region":
          return NextResponse.json({ cycle: await chooseRegionSuggestion(parent.id, { ...input, suggestion: body.suggestion as never }) });
        case "generate-origin":
          return NextResponse.json(await generateCharacterOriginSuggestions(parent.id, input));
        case "select-origin":
          return NextResponse.json({ cycle: await chooseOriginSuggestion(parent.id, { ...input, suggestion: body.suggestion as never }) });
        case "generate-saga":
          return NextResponse.json(await generateCoreSagaSuggestions(parent.id, input));
        case "select-saga":
          return NextResponse.json({ cycle: await chooseCoreSagaSuggestion(parent.id, { ...input, suggestion: body.suggestion as never }) });
        case "finalize": {
