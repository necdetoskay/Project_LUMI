  ...
  (request: Request, ctx: { params: Promise<{ id: string }> }) => {
    return withParent(async (parent) => {
      ...
      try {
        ...
        if (!body.fromOwnerType || !body.fromOwnerId || !body.toOwnerType || !body.toOwnerId) {
          return NextResponse.json(
            { error: "VALIDATION_ERROR", message: "fromOwnerType, fromOwnerId, toOwnerType, toOwnerId are required" },
            { status: 400 },
          );
        }
