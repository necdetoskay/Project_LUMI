...
const PRESET_MODELS = [
  { id: "aion-labs/aion-3.0-mini", label: "Aion 3.0 Mini (story/orijin önerileri)" },
  { id: "deepseek/deepseek-chat-v3-0324", label: "DeepSeek V3 0324 (ekonomik reasoning)" },
  { id: "deepseek/deepseek-chat-v3.1", label: "DeepSeek V3.1 (güçlü reasoning)" },
  { id: "mistralai/mistral-small-3.2-24b-instruct", label: "Mistral Small 3.2 24B (ucuz özet/açıklama)" },
  { id: "meta-llama/llama-guard-4-12b", label: "Llama Guard 4 12B (güvenlik kontrolü)" },
  { id: "meta-llama/llama-guard-3-8b", label: "Llama Guard 3 8B (güvenlik alternatifi)" },
  { id: "qwen/qwen3.5-flash-02-23", label: "Qwen 3.5 Flash (çok ucuz/hızlı)" },
  { id: "qwen/qwen3.6-flash", label: "Qwen 3.6 Flash (büyük context)" },
  { id: "google/gemini-2.5-flash-lite", label: "Gemini 2.5 Flash Lite (düşük maliyet)" },
  { id: "google/gemini-3.1-flash-lite-preview", label: "Gemini 3.1 Flash Lite Preview" },
  { id: "openai/gpt-oss-120b", label: "GPT OSS 120B (ekonomik yüksek reasoning)" },
...
const DEFAULT_TASK_CONFIGS: Array<{
  ...
  { taskType: "character_origin_generation", modelId: "aion-labs/aion-3.0-mini", reasoningLevel: "medium", temperature: 0.85, maxOutputTokens: 1800, enabled: true },
  { taskType: "story_outline_generation", modelId: "deepseek/deepseek-chat-v3-0324", reasoningLevel: "medium", temperature: 0.75, maxOutputTokens: 2400, enabled: false },
  { taskType: "story_turn_generation", modelId: "aion-labs/aion-3.0-mini", reasoningLevel: "high", temperature: 0.85, maxOutputTokens: 2200, enabled: false },
  { taskType: "safety_review", modelId: "meta-llama/llama-guard-4-12b", reasoningLevel: "low", temperature: 0, maxOutputTokens: 512, enabled: false },
  { taskType: "character_memory_summary", modelId: "mistralai/mistral-small-3.2-24b-instruct", reasoningLevel: "low", temperature: 0.3, maxOutputTokens: 900, enabled: false },
  { taskType: "parent_explanation", modelId: "mistralai/mistral-small-3.2-24b-instruct", reasoningLevel: "low", temperature: 0.4, maxOutputTokens: 900, enabled: false },
];
...
export default function LlmSettingsClientPage() {
  ...
  useEffect(() => {
    fetch("/api/onboarding")
      ...
      .then((data) => {
        ...
        if (hId) {
          setHouseholdId(hId);
          return fetch(`/api/settings/llm?householdId=${encodeURIComponent(hId)}`);
        }
  ...
  const handleSaveKey = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/settings/llm", {
        ...
        body: JSON.stringify({ action: "upsert-key", householdId, apiKey: apiKeyInput.trim() }),
      });
  ...
  const handleTestConnection = useCallback(async () => {
    if (!householdId) {
      setTestResult({ success: false, error: "Aile evreni bulunamadı. Önce kurulumu tamamlayın." });
      return;
    }
    setTesting(true);
    setTestResult(null);
    try {
      ...
      if (res.ok) {
        ...
        setTestResult({ success: false, error: json.message ?? "Test başarısız." });
      }
    } catch {
  ...
  const handleSaveTask = useCallback(async (taskType: string) => {
    if (!householdId) {
      showStatus("error", "Aile evreni bulunamadı. Önce kurulumu tamamlayın.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showStatus("success", `${TASK_LABELS[taskType] ?? taskType} ayarları kaydedildi.`);
      } else {
        showStatus("error", json.message ?? "Kaydetme başarısız.");
      }
    } catch {
  ...
  const updateFormField = useCallback(
    (taskType: string, field: keyof TaskSetting, value: string | number | boolean) => {
      setTaskForms((prev) => ({
        ...prev,
        [taskType]: { ...(prev[taskType] ?? {}), [field]: value } as TaskSetting,
      }));
    },
    [],
  );

  const initTaskConfig = useCallback(
    (taskType: string) => {
  ...
  const handleModelSelect = useCallback(
    (taskType: string, selectedId: string) => {
      if (selectedId === "__custom__") {
        ...
        updateFormField(taskType, "modelId", custom || "aion-labs/aion-3.0-mini");
      } else {
    ...
    return (
      <section className="mx-auto flex w-full max-w-[800px] flex-1 flex-col gap-8 px-6 py-10">
        <nav className="text-sm text-on-surface-variant">
          <a href="/app" className="hover:text-primary">Dashboard</a>
          <span className="mx-2">&gt;</span>
          <span className="text-on-surface">Ayarlar</span>
        </nav>
        <header>
          <h1 className="text-3xl font-extrabold tracking-tight text-on-surface">Ayarlar</h1>
          <p className="mt-2 text-on-surface-variant">OpenRouter bağlantısı ve görev bazlı model yapılandırması</p>
        </header>
    ...
    <section className="mx-auto flex w-full max-w-[800px] flex-1 flex-col gap-8 px-6 py-10">
      ...
      <section className="rounded-2xl border border-outline-variant bg-white p-6 md:p-8">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h2 className="text-xl font-bold text-on-surface">OpenRouter Bağlantısı</h2>
            <p className="mt-1 text-sm text-on-surface-variant">
              API anahtarını girerek LLM destekli karakter üretimini etkinleştir.
            </p>
          </div>
          <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${
            settings?.hasApiKey ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
          }`}>
      ...
      <section className="rounded-2xl border border-outline-variant bg-white p-6 md:p-8">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-on-surface">Görev Bazlı Model Ayarları</h2>
          <p className="mt-1 text-sm text-on-surface-variant">
        ...
        <div className="space-y-6">
          {DEFAULT_TASK_CONFIGS.map((defaultConfig) => {
            ...
            if (!form) {
              return (
                <div key={taskType} className="rounded-xl border border-dashed border-outline-variant bg-surface-container-low p-5">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-bold text-on-surface">{TASK_LABELS[taskType] ?? taskType}</h3>
                      <p className="mt-0.5 text-sm text-on-surface-variant">{TASK_DESCRIPTIONS[taskType] ?? ""}</p>
                    </div>
                    <button
            ...
            const isCustomModel = !PRESET_MODELS.some((m) => m.id === form.modelId);
            const selectedPreset = isCustomModel ? "__custom__" : form.modelId;

            return (
              <div key={taskType} className="rounded-xl border border-outline-variant bg-surface-container-low p-5">
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <label className="flex items-center gap-2 text-sm text-on-surface-variant">
                    <input
                      ...
                      onChange={(e) => updateFormField(taskType, "enabled", e.target.checked)}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div>
                    <label className="mb-1 block text-xs font-semibold text-on-surface-variant">Model</label>
                    <select
                      value={selectedPreset}
                      onChange={(e) => handleModelSelect(taskType, e.target.value)}
                    ...
                    {selectedPreset === "__custom__" && (
                      <input
                        ...
                        onChange={(e) => {
                          setCustomModelInputs((prev) => ({ ...prev, [taskType]: e.target.value }));
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-semibold text-on-surface-variant">Reasoning</label>
                    <select
                      value={form.reasoningLevel}
                      onChange={(e) => updateFormField(taskType, "reasoningLevel", e.target.value)}
                      className="w-full rounded-lg border border-outline-variant bg-white px-3 py-2 text-sm outline-none focus:border-primary"
                    >
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-semibold text-on-surface-variant">Temperature</label>
                    <input
                      type="number"
                      min="0"
                      max="2"
                      ...
                      onChange={(e) => updateFormField(taskType, "temperature", parseFloat(e.target.value) || 0)}
                      className="w-full rounded-lg border border-outline-variant bg-white px-3 py-2 text-sm outline-none focus:border-primary"
                    />
                  </div>

                  <div>
                    <label className="mb-1 block text-xs font-semibold text-on-surface-variant">Max Output Tokens</label>
                    <input
                      type="number"
                      min="256"
                      max="8000"
                      ...
                      onChange={(e) => updateFormField(taskType, "maxOutputTokens", parseInt(e.target.value, 10) || 256)}
                      className="w-full rounded-lg border border-outline-variant bg-white px-3 py-2 text-sm outline-none focus:border-primary"
                    />
                  </div>
                </div>
