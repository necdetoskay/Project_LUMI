...
export function ContextInspectorClientPage({
  ...
  useEffect(() => {
    fetch(`/api/settings/context-inspector/sessions?householdId=${householdId}`)
      .then((response) => {
        if (!response.ok)
          throw new Error("Generation sessions could not be loaded");
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((response) => {
        if (!response.ok)
          throw new Error("Generation history could not be loaded");
        return response.json();
      })
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((response) => {
        if (!response.ok)
          throw new Error("Inspection detail could not be loaded");
        return response.json();
      })
      .then((data: { inspection: InspectionDetail }) =>
      ...
      <header className="space-y-2">
        ...
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            Inspect what context was selected for a generation, how the token
            budget was used, and which findings affected the final model
            context.
          </p>
        </div>
      </header>
...
function InspectionDetailView({ detail }: { detail: InspectionDetail }) {
  const percentage = detail.tokenUsage.budget
    ? Math.min(
        100,
        Math.round((detail.tokenUsage.used / detail.tokenUsage.budget) * 100),
      )
    : 0;
    ...
    <>
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <dl className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <Info
            label="Context hash"
            value={detail.contextContentHash.slice(0, 16)}
          />
        </dl>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-xs text-slate-500">
          {detail.tokenUsage.used} used of {detail.tokenUsage.budget} available
          tokens
        </p>
      </div>
            ...
            {section.items?.length ? (
              <div className="space-y-2 border-t border-slate-100 p-4">
                {section.items.map((item, index) => (
                  <pre
                    key={index}
                    className="overflow-x-auto rounded-lg bg-slate-50 p-3 text-xs text-slate-700"
                  >
