...
const LAST_SEASON_HINT_KEY = "lumi.testLab.environmentSeasonHint";
...
export default function EnvironmentGenesisTestPanel() {
  ...
  const [initialSeasonHint, setInitialSeasonHint] = useState("");
  ...
  useEffect(() => {
    ...
    const rememberedSeason = window.localStorage.getItem(LAST_SEASON_HINT_KEY);
    if (rememberedModel?.trim()) setModelSlug(rememberedModel);
    if (rememberedLocale?.trim()) setLocale(rememberedLocale);
    if (rememberedSeason?.trim()) setInitialSeasonHint(rememberedSeason);
  ...
  async function previewPrompt() {
    ...
    try {
      ...
      window.localStorage.setItem(LAST_SEASON_HINT_KEY, initialSeasonHint);
      ...
      setMessage(
        "Environment Genesis production promptu ve başlangıç season context'i hazırlandı.",
      );
    } catch (error) {
      setMessage(
  ...
  async function runEnvironmentGenesis() {
    const context = sandboxContext();
    if (
      !prompt ||
      !context.sessionId ||
    ...
    try {
      ...
      setMessage(
        `${payload.data.candidates.length} Environment Genesis adayı üretildi; habitat persistence, climate compatibility ve source priority kontrolleri kaydedildi.`,
      );
    } catch (error) {
      setMessage(
  ...
  return (
    <section className={styles.shell} aria-label="Environment Genesis Test Lab">
      <section className={styles.panel}>
        ...
        <h2>Habitat · Climate · Season · Initial World State</h2>
        <p className={styles.muted}>
          Kalıcı habitat/iklim ile değişken season, weather ve day phase
          birbirinden ayrı tutulur. Kaynak önceliği world lore → region climate
          → universe calendar → real-world soft hint şeklindedir.
        </p>
        ...
        <div className={styles.settingsGrid}>
          ...
          <label className={styles.field}>
            Universe calendar başlangıç season hint
            <input
              className={styles.input}
              value={initialSeasonHint}
              placeholder="örn. winter veya Ayçiçeği Mevsimi"
              onChange={(event) => setInitialSeasonHint(event.target.value)}
            />
            <span className={styles.muted}>
              Bu bir ipucudur; world lore veya region climate varsa onları
              override edemez.
            </span>
          </label>
        </div>
        ...
        >
          Production Environment promptunu yükle
        </button>
        ...
        {prompt ? (
          <section className={styles.promptCard}>
            ...
            <button
              ...
              disabled={busy}
              onClick={runEnvironmentGenesis}
            >
              {busy ? "Çalışıyor..." : "Environment Genesis üret"}
            </button>
          </section>
        ) : null}
        ...
        <section className={styles.historySection}>
          <div className={styles.historyHeader}>
            <h3>Environment Genesis run sonuçları</h3>
            <button
              type="button"
              className={styles.secondaryButton}
              disabled={busy}
          ...
          {runs.length === 0 ? (
            <div className={styles.emptyState}>
              Henüz Environment Genesis runı yok.
            </div>
          ) : (
                ...
                return (
                  <article key={entry.run.id} className={styles.runCard}>
                    <div className={styles.runHeader}>
                      <strong>Environment Run {runs.length - index}</strong>
                    ...
                    <div className={styles.candidateList}>
                      {entry.candidates.map((candidate, candidateIndex) => (
                        ...
                        >
                          <strong>
                            Environment aday {candidateIndex + 1}
                          </strong>
                          <pre className={styles.payload}>
