...
const PHASE_ID = "character_genesis_inventory";
...
export default function InventoryGenesisTestPanel() {
  ...
  async function previewPrompt() {
    ...
    setMessage("");
    try {
      const payload = await post("/api/settings/test-lab/genesis/inventory", {
        action: "preview",
        ...context,
        modelSlug,
        locale,
  ...
  async function runInventoryGenesis() {
    ...
    try {
      ...
      setMessage(
        `${payload.data.candidates.length} Inventory Genesis adayı üretildi; canonical item manifest ve provenance kaydedildi.`,
      );
    } catch (error) {
      setMessage(
  ...
  async function selectCandidate(entry: RunEntry, candidate: Candidate) {
    ...
    try {
      ...
      await refreshHistory();
      setMessage("Inventory Genesis adayı sandbox state için seçildi.");
    } catch (error) {
  ...
  return (
    <section className={styles.shell} aria-label="Inventory Genesis Test Lab">
      <section className={styles.panel}>
        ...
        >
          Production Inventory Genesis promptunu yükle
        </button>
        ...
        {prompt ? (
          <section className={styles.promptCard}>
            ...
            <button
              ...
              disabled={busy}
              onClick={runInventoryGenesis}
            >
              {busy ? "Çalışıyor..." : "Inventory Genesis üret"}
            </button>
          </section>
        ) : null}
        ...
        <section className={styles.historySection}>
          <div className={styles.historyHeader}>
            <h3>Inventory Genesis run sonuçları</h3>
            <button
              type="button"
              className={styles.secondaryButton}
              disabled={busy}
