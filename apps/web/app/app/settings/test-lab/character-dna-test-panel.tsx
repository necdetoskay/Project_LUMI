...
const PHASE_ID = "character_genesis_character_dna";
...
export default function CharacterDnaTestPanel() {
  ...
  async function previewPrompt() {
    ...
    setMessage("");
    try {
      const payload = await post("/api/settings/test-lab/genesis/dna", {
        action: "preview",
        ...context,
        modelSlug,
        locale,
  ...
  async function selectCandidate(entry: RunEntry, candidate: Candidate) {
    ...
    try {
      ...
      await refreshHistory();
      setMessage("Character DNA adayı sandbox state için seçildi.");
    } catch (error) {
  ...
  return (
    ...
    >
      <section className={styles.panel}>
        ...
        >
          Production Character DNA promptunu yükle
        </button>
        ...
        {prompt ? (
          <section className={styles.promptCard}>
            ...
            <button
              ...
              disabled={busy}
              onClick={runCharacterDna}
            >
              {busy ? "Çalışıyor..." : "Character DNA üret"}
            </button>
          </section>
        ) : null}
        ...
        <section className={styles.historySection}>
          <div className={styles.historyHeader}>
            <h3>Character DNA run sonuçları</h3>
            <button
              type="button"
              className={styles.secondaryButton}
              disabled={busy}
                ...
                return (
                  <article key={entry.run.id} className={styles.runCard}>
                    <div className={styles.runHeader}>
                      <strong>Character DNA Run {runs.length - index}</strong>
                        ...
                        >
                          ...
                          >
                            {candidate.id === entry.selectedCandidateId
                              ? "Seçilen Character DNA"
                              : "Bu Character DNA adayını seç"}
                          </button>
                        </article>
                      ))}
                    </div>
