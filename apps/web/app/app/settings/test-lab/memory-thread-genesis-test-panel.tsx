...
const PHASE_ID = "character_genesis_memory_threads";
...
export default function MemoryThreadGenesisTestPanel() {
  ...
  async function runMemoryThreads() {
    const context = sandboxContext();
    if (
      !prompt ||
      !context.sessionId ||
    ...
    try {
      ...
      setMessage(
        `${payload.data.candidates.length} Memory/Thread adayı üretildi; visibility, duplicate/contradiction ve future-story-yield kontrolleri kaydedildi.`,
      );
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "Memory Seeds & Origin Threads çalıştırılamadı.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function selectCandidate(entry: RunEntry, candidate: Candidate) {
    ...
    try {
      ...
      await refreshHistory();
      setMessage("Memory/Thread adayı sandbox state için seçildi.");
    } catch (error) {
    ...
    <section className={styles.shell} aria-label="Memory Seeds Origin Threads Test Lab">
      <section className={styles.panel}>
        ...
        <p className={styles.muted}>
          Deep Origin&apos;den 3-5 retrievable geçmiş anısı ve hikâyeler boyunca
          yaşayabilen unresolved thread&apos;ler çıkarır. Hidden thread&apos;ler
          character-visible context&apos;e giremez; thread kullanımı planner
          tarafında tekrar kullanım cezasıyla ağırlıklandırılır.
        </p>
        ...
        >
          Production Memory/Thread promptunu yükle
        </button>
        ...
        {prompt ? (
          <section className={styles.promptCard}>
            ...
            <button
              ...
              disabled={busy}
              onClick={runMemoryThreads}
            >
              {busy ? "Çalışıyor..." : "Memory Seeds & Origin Threads üret"}
            </button>
          </section>
        ) : null}
        ...
        <section className={styles.historySection}>
          <div className={styles.historyHeader}>
            <h3>Memory/Thread run sonuçları</h3>
            <button
              type="button"
              className={styles.secondaryButton}
              disabled={busy}
                ...
                return (
                  <article key={entry.run.id} className={styles.runCard}>
                    <div className={styles.runHeader}>
                      <strong>Memory/Thread Run {runs.length - index}</strong>
