...
export function AssetsClientPage({
  ...
  const [state, setState] = useState<LibraryResponse>({ canon: null, candidates: [] });
  ...
  const refresh = useCallback(async () => {
    ...
    const payload = (await response.json()) as LibraryResponse & { error?: string };
    if (!response.ok) throw new Error(payload.error ?? "Görsel kütüphanesi okunamadı.");
    setState(payload);
  }, [endpoint]);

  useEffect(() => {
    void refresh().catch((error: unknown) => {
      setMessage(error instanceof Error ? error.message : "Görseller yüklenemedi.");
    });
  }, [refresh]);
      ...
      <section className="storybook-page min-h-full">
        <div className="mx-auto w-full max-w-[920px] px-5 py-10">
          <div className="rounded-[2rem] border border-outline-variant/70 bg-white/85 p-8">
            <h1 className="text-3xl font-extrabold text-on-surface">Görsel Kütüphanesi</h1>
            <p className="mt-3 text-on-surface-variant">
              Önce aile alanınızı oluşturun; karakter görselleri daha sonra burada yönetilecek.
            </p>
      ...
      <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-6 px-5 py-8 md:px-6 md:py-10">
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-[48rem] leading-7 text-on-surface-variant">
                Yeni görseller oluşturabilir, adayları karşılaştırabilir ve karakterin hikâyelerde kullanılacak kalıcı görünümünü seçebilirsiniz.
              </p>
            </div>
        ...
        {characters.length === 0 ? (
          <div className="rounded-[2rem] border border-dashed border-outline-variant bg-white/75 p-8 text-center">
            <h2 className="text-2xl font-extrabold text-on-surface">Henüz karakter yok</h2>
            <p className="mt-3 text-on-surface-variant">
              Bir çocuk profili için karakter oluşturduğunuzda görsel adayları burada görünecek.
            </p>
          </div>
        ) : (
          <>
            <section className="rounded-[2rem] border border-outline-variant/70 bg-white/80 p-6 shadow-sm">
              <label className="text-sm font-extrabold text-on-surface" htmlFor="visual-character">
                Karakter
              </label>
              <select
                id="visual-character"
            ...
            <section aria-labelledby="visual-candidates-heading">
              <div className="mb-4">
                ...
                <h2 id="visual-candidates-heading" className="mt-2 text-3xl font-extrabold text-on-surface">
                        ...
                        <div className="p-5">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="rounded-full bg-surface-container px-3 py-1 text-xs font-bold text-on-surface-variant">
                              {isCanon ? "Seçili görünüm" : candidate.lifecycleState}
                            </span>
                          </div>
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            {!isCanon && candidate.lifecycleState !== "rejected" ? (
                              <button
                                className="storybook-button"
                                disabled={busy}
                                onClick={() => void act({ action: "select", assetId: candidate.id })}
                                type="button"
                              >
                                Bu görünümü seç
                              </button>
                            ...
                            {!isCanon && candidate.lifecycleState !== "rejected" ? (
                              <button
                                className="storybook-button-secondary"
                                disabled={busy}
                                onClick={() => void act({ action: "reject", assetId: candidate.id })}
                                type="button"
                              >
                                Ele
                              </button>
