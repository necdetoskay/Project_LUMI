...
export default function ProfileExperienceClientPage({
    ...
    async function load() {
      try {
        ...
        const [profileResponse, characterResponse, personalizationResponse] =
          await Promise.all([
            fetch(
              `/api/child-profiles/${encodeURIComponent(childProfileId)}?${query}`,
            ),
            fetch(
  ...
  if (loading) {
    return (
      <StateCard icon="auto_awesome" message="Çocuğun dünyası hazırlanıyor…" />
    );
  }
  if (error || !profile) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1240px] flex-col gap-7 px-4 py-6 sm:px-6 lg:py-9">
      ...
      <section className="relative overflow-hidden rounded-[32px] border border-primary/15 bg-[radial-gradient(circle_at_top_left,_rgba(109,74,255,0.22),_transparent_38%),linear-gradient(135deg,#fbf9ff_0%,#f2efff_55%,#fff8ee_100%)] p-6 shadow-sm md:p-9">
        ...
        <div className="pointer-events-none absolute bottom-0 right-16 text-[110px] opacity-[0.08]">
          ✨
        </div>
        <div className="relative grid gap-7 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center">
          <div className="flex items-start gap-5">
            ...
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.16em] text-primary">
                {profile.displayName}’in LUMI alanı
              </p>
              <h1 className="mt-2 text-3xl font-extrabold tracking-tight text-on-surface md:text-5xl">
                Onu tanıdıkça hikâyeler de değişsin.
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-on-surface-variant md:text-lg">
                İlgi alanlarını ve desteklemek istediğiniz gelişim alanlarını
                burada güncelleyin. LUMI bu bilgileri çocuğu etiketlemek için
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <InfoPill
                  icon="psychiatry"
                  text={`${personalization.developmentGoals.length} destek hedefi`}
                />
                {firstCharacter ? (
                  <InfoPill icon="auto_awesome" text={firstCharacter.name} />
                ) : null}
              </div>
            </div>
          </div>
          <div className="grid gap-3 rounded-[24px] border border-white/70 bg-white/65 p-5 backdrop-blur">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">
              Dünyasına hızlı geçiş
            </p>
            <Link
              className="flex items-center justify-between rounded-2xl bg-primary px-5 py-4 font-bold text-on-primary shadow-sm transition-transform hover:-translate-y-0.5"
              href={`/app/profiles/${encodeURIComponent(childProfileId)}/world`}
            >
              <span>🌍 Dünyasını aç</span>
              <span>→</span>
            </Link>
            {firstCharacter ? (
              <button
                className="flex items-center justify-between rounded-2xl border border-outline-variant bg-white px-5 py-4 text-left font-bold text-on-surface"
                type="button"
              ...
              <Link
                className="flex items-center justify-between rounded-2xl border border-outline-variant bg-white px-5 py-4 font-bold text-on-surface"
                href={`/app/character-onboarding?childProfileId=${encodeURIComponent(childProfileId)}`}
              >
                <span>✨ İlk karakteri oluştur</span>
                <span>→</span>
              </Link>
            )}
          </div>
      ...
      {activeTab === "personalization" ? (
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            <Panel
              eyebrow="❤️ Neleri seviyor?"
              title="İlgi alanları"
              description="Hikâyelerin dünyasını, karşılaşmaları ve temaları çocuğun sevdiği şeylerle daha anlamlı hale getirelim."
            >
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                {INTERESTS.map(([value, emoji, label]) => (
                  <ChoiceCard
                    key={value}
                    selected={personalization.interests.includes(value)}
              ...
              <div className="mt-5 rounded-2xl bg-surface-container-low p-4">
                ...
                <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-xl border border-outline-variant bg-white px-4 py-3 outline-none focus:border-primary"
                    placeholder="Örn: trenler, iş makineleri, gökyüzü…"
                  />
                  <button
                    type="button"
                    onClick={addCustomInterest}
                    className="rounded-xl bg-white px-5 py-3 font-bold text-primary shadow-sm"
                  >
                    Ekle
                  </button>
                </div>
            ...
            <Panel
              eyebrow="🌱 Neleri destekleyelim?"
              title="Gelişim hedefleri"
              description="Bu hedefler çocuğa bir etiket veya puan olarak gösterilmez. LUMI bunları hikâyelerin içine doğal seçimler ve deneyimler olarak yerleştirir."
            >
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {GOALS.map(([value, emoji, label]) => (
                  <ChoiceCard
                    key={value}
                    selected={personalization.developmentGoals.includes(value)}
            ...
            <div className="flex flex-col items-start justify-between gap-3 rounded-2xl border border-primary/15 bg-primary/5 p-5 sm:flex-row sm:items-center">
              <div>
                <p className="font-bold text-on-surface">
                  Değişiklikleri hikâye motoruna aktar
                </p>
                <p className="mt-1 text-sm text-on-surface-variant">
                  Kaydedilen tercihler profil metadata’sında kalıcı olarak
                  tutulur ve hikâye kişiselleştirme girdisi olarak
                  kullanılabilir.
                </p>
                {saveMessage ? (
              ...
              <button
                ...
                className="rounded-xl bg-primary px-6 py-3 font-bold text-on-primary shadow-sm disabled:opacity-50"
              >
          ...
          <aside className="space-y-5">
            <Panel
              eyebrow="✨ LUMI’nin fark ettikleri"
              title="Gözlemler"
              description="Hikâyeler ilerledikçe çocuğun tercihleriyle ilgili betimleyici, ebeveyn odaklı gözlemler burada görünecek."
            >
              <div className="rounded-2xl border border-dashed border-outline-variant bg-surface-container-low p-5 text-sm leading-6 text-on-surface-variant">
                Henüz yeterli hikâye gözlemi yok. LUMI burada “iyi/kötü”
                puanları üretmeyecek; örneğin hangi temalara daha çok yöneldiği
                veya iş birliği gerektiren durumlarda nasıl seçimler yaptığı
                gibi açıklanabilir gözlemler sunacak.
              </div>
            </Panel>
            <Panel
              eyebrow="📖 Hikâye yaklaşımı"
              title="Doğal öğrenme"
              description="Gelişim hedefleri ders başlığına dönüşmez."
            >
              <p className="text-sm leading-6 text-on-surface-variant">
                Örneğin “paylaşma” hedefinde doğrudan nasihat vermek yerine tek
                bir feneri iki karakterin birlikte kullanması gibi doğal bir
                hikâye durumu oluşturulur. Çocuk özgürce karar verir; sistem
                yalnızca fırsat sunar.
              </p>
            </Panel>
          </aside>
        </section>
      ...
      {activeTab === "characters" ? (
        <section className="grid gap-5 md:grid-cols-2">
          {characters.length === 0 ? (
            <Panel
              eyebrow="✨ Karakterler"
              title="İlk kahraman henüz doğmadı"
              description="Çocuğun dünyasına eşlik edecek ilk karakteri birlikte oluşturalım."
            >
              <Link
                className="inline-flex rounded-xl bg-primary px-5 py-3 font-bold text-on-primary"
                href={`/app/character-onboarding?childProfileId=${encodeURIComponent(childProfileId)}`}
              >
                Karakter oluştur
              </Link>
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <p className="mt-2 text-sm text-on-surface-variant">
                    {character.broadKind} · {character.subtype}
                  </p>
                </div>
              </article>
            ))
...
function Overview({
  ...
  profile: Profile;
  personalization: Personalization;
  firstCharacter: CharacterInfo | null;
  onOpenPersonalization: () => void;
}) {
  const interestLabels = [
    ...personalization.interests
      .map((value) => INTERESTS.find(([key]) => key === value)?.[2])
      .filter(Boolean),
    ...personalization.customInterests,
  ];
  const goalLabels = personalization.developmentGoals
    .map((value) => GOALS.find(([key]) => key === value)?.[2])
    .filter(Boolean);
  return (
    <section className="grid gap-6 lg:grid-cols-2">
      <Panel
        eyebrow="❤️ Onu tanıyalım"
        title="Sevdiği dünyalar"
        description="Hikâyelerin çocuğa tanıdık ve heyecan verici gelmesi için ilgi alanlarını güncel tutun."
      >
        {interestLabels.length ? (
          <div className="flex flex-wrap gap-2">
            {interestLabels.map((label) => (
              <span
                key={label}
                className="rounded-full bg-primary/8 px-3 py-2 text-sm font-bold text-primary"
              >
                {label}
              </span>
            ))}
          ...
          <EmptyLine text="Henüz ilgi alanı seçilmedi." />
        )}
        <button
          type="button"
      ...
      <Panel
        eyebrow="🌱 Ebeveyn rehberliği"
        title="Desteklemek istediğiniz alanlar"
        description="Hedefler hikâyelere doğal seçim fırsatları olarak yansır; çocuğa eksiklik olarak gösterilmez."
      >
        {goalLabels.length ? (
          <div className="flex flex-wrap gap-2">
            {goalLabels.map((label) => (
              <span
                key={label}
                className="rounded-full bg-[#fff4dc] px-3 py-2 text-sm font-bold text-[#7a4f00]"
              >
                {label}
              </span>
            ))}
          ...
          <EmptyLine text="Henüz gelişim hedefi seçilmedi." />
        )}
        <button
          type="button"
          onClick={onOpenPersonalization}
      ...
      <Panel
        ...
        title={firstCharacter?.name ?? "Karakter bekleniyor"}
        description={
          firstCharacter
            ? "Bu karakter çocuğun hikâyelerindeki devamlılığın önemli parçalarından biri."
            : "İlk karakter oluşturulduğunda burada çocuğun yol arkadaşı görünecek."
        }
      >
        <p className="text-sm leading-6 text-on-surface-variant">
          Profil yaşı:{" "}
      ...
      <Panel
        eyebrow="🔎 LUMI’nin fark ettikleri"
        title="Zamanla öğrenen profil"
        description="Sistem hikâyelerdeki seçimleri puanlamak yerine anlamlı eğilimleri ebeveyne açıklayacak."
      >
...
function Panel({
  ...
  return (
    <section className="rounded-[28px] border border-outline-variant bg-white p-6 shadow-sm md:p-7">
      <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">
        {eyebrow}
      </p>
...
function ChoiceCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`flex ${compact ? "min-h-20" : "min-h-28"} flex-col items-start justify-between rounded-2xl border p-4 text-left transition-all ${selected ? "border-primary bg-primary/8 shadow-sm ring-2 ring-primary/10" : "border-outline-variant bg-white hover:-translate-y-0.5 hover:border-primary/30"}`}
    >
      <span className="text-2xl">{emoji}</span>
      <span className="mt-3 text-sm font-bold text-on-surface">{label}</span>
      {selected ? <span className="absolute" /> : null}
    </button>
  );
}
...
function Tab({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${active ? "bg-white text-primary shadow-sm" : "text-on-surface-variant hover:text-on-surface"}`}
    >
      {children}
    </button>
  );
...
function InfoPill({ icon, text }: { icon: string; text: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/80 bg-white/70 px-3 py-1.5 text-sm font-semibold text-on-surface-variant">
...
function StateCard({ icon, message }: { icon: string; message: string }) {
  return (
    <main className="mx-auto flex min-h-[60vh] w-full max-w-[900px] items-center justify-center px-6">
      <div className="rounded-[28px] border border-outline-variant bg-white px-8 py-10 text-center shadow-sm">
        <span className="material-symbols-outlined text-4xl text-primary">
          {icon}
        </span>
        <p className="mt-4 font-semibold text-on-surface">{message}</p>
      </div>
    </main>
  );
}
