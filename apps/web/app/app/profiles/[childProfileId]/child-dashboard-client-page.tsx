        ...
        <aside className="border-b border-[#e8dcc8] bg-[#fffaf0] px-4 py-5 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:px-5 lg:py-7">
          ...
          <div className="mt-8 hidden rounded-[28px] bg-[linear-gradient(160deg,#edf5ea,#fff5dc)] p-5 text-sm text-[#65584d] lg:block">
            ...
            <p className="mt-1 leading-6">
              Yeni izler, hikâyeler ve keşifler burada birikir.
            </p>
          </div>
        </aside>
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
              <div className="flex min-w-0 items-center gap-4 sm:gap-5">
                {primaryCharacter ? (
                  ...
                  <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full bg-[#f0eadf] text-4xl sm:h-24 sm:w-24">
                    🌱
                  </div>
                )}
                <div className="min-w-0">
                  <p className="text-sm font-bold text-[#1f7a70]">
                    Çocuğun LUMI alanı
                  </p>
                  <h1 className="mt-1 truncate text-3xl font-black tracking-tight sm:text-4xl">
                    Merhaba,{" "}
                    {primaryCharacter?.name ?? data.profile.displayName} ✨
                  </h1>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <InfoPill
                      icon="cake"
                      text={ageLabel(data.profile.ageBand)}
                    />
                    {worldLabel ? (
                      <InfoPill icon="forest" text={worldLabel} />
                    ) : null}
                    <InfoPill
                      icon="favorite"
                      text={`${interestCount} ilgi alanı`}
                    />
                  </div>
                </div>
              </div>
          ...
          {ongoingAdventure && primaryCharacter ? (
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {currentLocation ? (
                    <StoryChip
                      icon="location_on"
                      text={`En son: ${currentLocation}`}
                    />
                  ) : null}
                  {meaningfulItem ? (
                    <StoryChip
                      icon="backpack"
                      text={`Yanında: ${meaningfulItem.displayName}`}
                      warm
                    />
                  ) : null}
                </div>
            ...
            <div className="p-7 sm:p-9">
              ...
              <p className="mt-2 max-w-xl leading-7 text-[#6b5e53]">
                Henüz devam eden bir macera yok. Dünya hazır olduğunda ilk
                hikâyeni buradan başlatabilirsin.
              </p>
              <button
                type="button"
                onClick={onOpenStories}
                className="mt-5 rounded-2xl bg-[#16786f] px-5 py-3 font-black text-white"
              >
                Hikâyelere Git
              </button>
            </div>
        ...
        <section className="rounded-[28px] border border-[#eadfce] bg-[#fffdf9] p-5 shadow-sm sm:p-6">
          ...
          {primaryCharacter ? (
            <button
              ...
              className="mt-5 grid w-full overflow-hidden rounded-[22px] border border-[#dfd5ee] bg-white text-left sm:max-w-xl sm:grid-cols-[180px_1fr]"
            >
              <CanonicalCharacterImage
                characterId={primaryCharacter.id}
                householdId={householdId}
            ...
            <div className="mt-5 rounded-[22px] border border-dashed border-[#d9ccba] bg-[#fffaf1] p-7 text-center">
              ...
              <button
                type="button"
                onClick={onOpenCharacters}
                className="mt-3 text-sm font-black text-[#6c42df]"
              >
          ...
          <section className="rounded-[28px] border border-[#eadfce] bg-[#fffdf9] p-5 shadow-sm sm:p-6">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={onOpenStories}
                className="text-sm font-black text-[#6c42df]"
              >
                Tümünü Gör →
              </button>
            </div>
            <div className="mt-5 grid gap-4 md:grid-cols-3">
              {pastAdventures.slice(0, 3).map((adventure) => (
                <article
                  ...
                  className="overflow-hidden rounded-[22px] border border-[#eadfce] bg-white"
                >
                  <div className="grid h-28 place-items-center bg-[linear-gradient(135deg,#dceee6,#f7e8c8)] text-4xl">
                    ✨
                  </div>
                  <div className="p-4">
                    <h3 className="font-black leading-5">{adventure.title}</h3>
                    <p className="mt-2 line-clamp-3 text-xs leading-5 text-[#6f6258]">
      ...
      <div className="space-y-6">
          ...
          {suggestions.length > 0 ? (
            <div className="mt-5 space-y-4">
              ...
              <button
                type="button"
                onClick={onOpenStories}
                className="w-full rounded-2xl border border-[#dfd3c1] bg-white px-4 py-3 text-sm font-black"
              >
            ...
            <div className="mt-5 rounded-[22px] border border-dashed border-[#ddd0bd] bg-[#fffaf1] p-6 text-sm leading-6 text-[#6d6056]">
              Dünya biraz sessiz görünüyor. Yeni bir macera fikri için Hikâyeler
              bölümüne geçebilirsin.
            </div>
          )}
        </section>
        ...
        <section className="rounded-[28px] border border-[#eadfce] bg-[#fffdf9] p-5 shadow-sm sm:p-6">
          ...
          {inventory.length > 0 ? (
            ...
            <p className="mt-4 rounded-2xl bg-[#fffaf1] p-4 text-sm text-[#70645a]">
              Çantada henüz gösterilecek bir eşya yok.
            </p>
          )}
        </section>
      </div>
      ...
      {characters.length > 0 ? (
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {characters.map((character) => (
            <article
              key={character.id}
              className="overflow-hidden rounded-[24px] border border-[#e1d7c7] bg-white shadow-sm"
            >
              <CanonicalCharacterImage
        ...
        <div className="mt-7 rounded-[24px] border border-dashed border-[#ddcfbc] bg-[#fff9ee] p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#71645a]">
            Dünya ve hikâye yolculuğu karakterinle başlayacak.
          </p>
        </div>
      )}
    </section>
  ...
  return (
    <section className="mt-6 rounded-[30px] border border-[#eadfce] bg-[#fffdf9] p-5 shadow-sm sm:p-7">
      ...
      {inventory.length > 0 ? (
        ...
        <div className="mt-6 rounded-[22px] border border-dashed border-[#ddcfbc] bg-[#fff9ee] p-8 text-center text-[#70645a]">
          Çanta henüz boş.
        </div>
      )}
    </section>
  ...
  return (
    <article className={`rounded-[22px] border p-4 ${presentation.surface}`}>
      <div className="flex gap-4">
        <div className="grid h-20 w-20 shrink-0 place-items-center rounded-[18px] bg-white/80 text-3xl shadow-sm">
          {presentation.emoji}
        </div>
        <div className="min-w-0 flex-1">
          ...
          <button
            type="button"
            onClick={onClick}
            className={`mt-3 rounded-xl px-3 py-2 text-xs font-black text-white ${presentation.button}`}
          >
            {presentation.cta}
          </button>
        </div>
...
function NavButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex min-w-max items-center gap-3 rounded-2xl px-4 py-3 text-sm font-black transition lg:w-full ${active ? "border border-[#cfdccb] bg-[#edf5ef] text-[#176f66]" : "text-[#5d5147] hover:bg-[#f5eee3]"}`}
    >
...
function InfoPill({ icon, text }: { icon: string; text: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-[#e8ddcb] bg-white px-3 py-1.5 text-xs font-bold text-[#5e5248]">
      <span className="material-symbols-outlined text-[16px] text-[#6c42df]">
        {icon}
      </span>
      {text}
    </span>
...
function StoryChip({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-2 text-xs font-bold ${warm ? "border-[#ead2a1] bg-[#fff3cf]" : "border-[#c9ded5] bg-[#eef8f3]"}`}
    >
  ...
  return (
    <main className="grid min-h-[70vh] place-items-center bg-[#f8f4ea] p-6">
      <div className="rounded-[28px] border border-[#eadfce] bg-white px-8 py-10 text-center shadow-sm">
        <span className="material-symbols-outlined text-4xl text-[#1f7a70]">
          {icon}
        </span>
...
function suggestionPresentation(kind: string) {
  if (kind === "inventory") {
    return {
      label: "Çantandaki Bir Eşya",
      emoji: "🎒",
      cta: "Eşyayı takip et",
      surface: "border-[#ead7aa] bg-[#fff9e9]",
      text: "text-[#9a651d]",
      button: "bg-[#d99725]",
    };
  }
  if (kind === "origin") {
    return {
      ...
      emoji: "🧭",
      cta: "İzin peşine düş",
      surface: "border-[#d9cce8] bg-[#faf5ff]",
      text: "text-[#6d4a97]",
      button: "bg-[#8a67b3]",
    };
  }
  return {
    label: "Dünyada Bir Şey Oldu",
    emoji: "🌍",
    cta: "Bu macerayı seç",
    surface: "border-[#c9ded5] bg-[#f2faf6]",
    text: "text-[#176f66]",
    button: "bg-[#1c8277]",
  };
}
