...
- Sprint 21 — Stability, Performance and Scale [devam ediyor] (spec: `docs/07-delivery/lumi/sprint-21/SPRINT_SPEC.md`; S21-T01 load harness scaffold [PR #26 carry]: `apps/web/tests/load/session-advance.load.test.ts` + multi-project `vitest.config.ts` (ui jsdom + load node); S21-T02 latency/error/isolation SLIs + CI gate [PR #26 carry]: non-blocking `pnpm --filter @lumi/web test:load` step in `ci.yml`; S21-T03..T05 pending) (spec: `docs/07-delivery/lumi/sprint-20/SPRINT_SPEC.md`; S20-T01 scope freeze/traceability [committed `codex/sprint-20-scope-freeze`]: no `TODO/FIXME` in source, all workspace pkgs/version pinned at `0.1.0`, commit→PR→Sprint traceability mapped for S09–S19, root `CHANGELOG.md` seeded; S20-T02 version/build artifact reproducibility [merged PR #26]: `.github/workflows/ci.yml` `build` job (docker/metadata-action + build-push-action, version+sha tags, OCI labels, tags-only no-push) + Dockerfile runner-stage `IMAGE_VERSION`/`IMAGE_COMMIT_SHA` ARGs + `org.opencontainers.image.*` labels; S20-T03 migration/restore rehearsal [merged PR #27]: `docs/ops/runbooks/staging-migration-drill.md` — additive-migration drill against a throwaway staging snapshot with a forward-fix/restore (no rollback) decision matrix; S20-T04 full release test suite [PR #28]: 106 test files, 137/138 pass (1 flaky isolated-green `getByText` collision in `@lumi/web`), IDOR regression coverage on 6 session mutators, all 13 integration suites `*_TEST_ENABLE_DESTRUCTIVE`-guarded, load/stress gap logged; S20-T05 runbooks/monitoring/handoff [PR #28 follow-up]: `docs/ops/runbooks/release-runbook.md` (SHA-bound RC tagging, reproducible build steps, human approval gate, forward-fix-only recovery) + `docs/ops/runbooks/release-monitoring.md` (health/latency/error/isolation thresholds, escalation, deploy handoff checklist); S20-T06 RC PR/tag + approval package ready (release-runbook.md SHA-bound RC tag + human approval gate + CHANGELOG 0.1.0 entry; pending owner SHA-bound sign-off on the exact commit)
