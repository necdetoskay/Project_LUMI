...
- Sprint 31 — Quest Seed Automation (template → quest) [tamamlandı] (spec: `docs/07-delivery/lumi/sprint-31/SPRINT_SPEC.md`; S31-T01 `QuestSeedTemplateResolver` ✅ (`@lumi/world` domain — deterministik `factId→templateKey`, seeded registry `lost-letter-quest`/`bridge-repair-quest` + stable default fallback, `assertKnownQuestSeedTemplateKey`), 6 unit test; S31-T02 `QuestSeedAutomationService.instantiateQuestFromSeed` ✅ (template resolve → `instantiateQuestFromTemplate` → `activateQuest`, `world_idempotency_ledger` `quest_seed_automation`/`quest-seed:<hookId>` idempotency — re-run aynı quest, no duplicate), 4 unit + guarded integration; S31-T03 story `quest_seed_automation` outbox intent ✅ (`OUTBOX_INTENT_TYPES` + `StoryHookService.createHook` quest*seed hook için atomik enqueue — payload hookId/opportunityId/storySessionId/worldId/householdId/factId/sourceNpcId, story world import etmez), 3 yeni story-hook test; S31-T04 `QuestSeedAutomationApplicator` ✅ (`@lumi/world` — intent+payload validation, `instantiateQuestFromSeed` delegasyonu, idempotent) + migration `0008_quest_seed_automation.sql` (template+objective seed), 4 unit test; S31-T05 backlog validation evidence ✅: `docs/07-delivery/lumi/sprint-31/S31_T05_VALIDATION_EVIDENCE.md`). `@lumi/world` 130 unit + `@lumi/story` 115 unit green; `pnpm format:check | lint | typecheck | test | build` + check-mojibake green. Kapsar: S30 closeout'tan kalan \_quest_seed → quest automation* backlog adımı.)
...
- Migration'lar DB'ye uygulandı (2026-08-18, production Neon): story `0008`, world `0010` + follow-up `0009_story_session_characters_cascade.sql` (story) ve `0011_world_character_cascade.sql` (world).
- **Delete 500 fix (2026-08-18)**: kalıcı silme "bazen" 500 veriyordu çünkü cascade migration'ları DB'ye uygulanmamıştı; `profile.worlds.child_profile_id` `NO ACTION` kalıp FK violation üretiyordu. `0008`/`0010` uygulanınca sıra bağımlı iki ek `NO ACTION` FK daha blokluyordu — `0011_world_character_cascade.sql` (`profile.worlds.character_id`, `world_character_locations.character_id`, `world_residences.character_id` → `lumi_characters` CASCADE) ve `0009_story_session_characters_cascade.sql` (`story_session_characters.character_id` → `lumi_characters` CASCADE). Uygulandıktan sonra child-delete closure'ındaki TÜM FK'lar CASCADE/SET NULL; production rollback'lı test DELETE'leri (dünya+karakter+görsel asset+story session/definition verisi olan çocuklar) başarılı. Not: `story_hooks`/`story_commit_records`/`story_world_versions`/`story_generation_inspections` FK'sız olduğundan cascade ile silinmez (orphan kalır, silme başarılı) — temizlik ayrı faz olarak backlog'a eklenebilir.

### Header auth-aware nav (2026-08-18)

- Kullanıcı isteği: login olmadan üstteki butonlar görünmemeli. `AppHeader` server component artık oturum kontrolü yapıyor (`getParentFromSessionToken` + `getParentSessionCookie`).
- Login yokken: logo + dil seçici + "Hikâyeme dön" (login link) görünür; nav butonları (Ana Sayfa/Çocuklarım/Görsel Kütüphanesi/Güvenlik) ve ayarlar dişlisi gizlenir. Login varken: logo + nav + dil seçici + ayarlar dişlisi (login link gizlenir).
- **Production 500 fix (2026-08-18, `65b5d0af`)**: `AppHeader` async server component'e çevrilirken `useTranslations` (React hook, içinde `use()` ile suspend eder) `next-intl`'in client hook'undan kalıyordu; async RSC içinde hook çağrısı `Expected a suspended thenable` hatasıyla tüm root layout'u (landing page `/`) 500'e düşürüyordu (local + Vercel production). Fix: `useTranslations` → `getTranslations` (`next-intl/server`). Kural: **async server component'lerde asla `useTranslations` hook'u kullanılmaz** — `next-intl/server`'dan `await getTranslations` kullanılır; senkron RSC'lerde hook OK.

### Character creation wizard cancel (2026-08-18)

- Kullanıcı isteği: yarım kalan karakter oluşturma sihirbazı kaldığı yerden devam edebiliyor; artık istenirse **iptal edilebilmeli** (önceden yalnızca devam edilebiliyordu — draft hep yakalanıyordu).
- Sebep: sihirbaz ilerlemesi sunucuda `character_creation_cycles` (`status='draft'`) + `character_creation_selections`'ta tutuluyor; schema'da `abandoned`/`abandoned_at` tanımlıydı ama hiçbir kod yazmıyordu; UI'da iptal butonu yoktu; partial unique index (child başına tek draft) "yeniden başla"yı da engelliyordu.
- `@lumi/profiles`: `abandonCharacterCreationCycle(userId, householdId, childProfileId)` — scope assert + aktif draft yoksa `CHARACTER_CREATION_CYCLE_REQUIRED`, varsa `status='abandoned'` + `abandoned_at` yazar. `application/index.ts` export.
