...
- Sprint 22 — Story Outcome & World State Commit System [devam ediyor] (spec: `docs/07-delivery/lumi/sprint-22/SPRINT_SPEC.md`; S22-T01 outcome manifest + story context snapshot ✅: `@lumi/story` domain modelleri `OutcomeManifest` (typed changes + evidence + idempotency keys) ve `StoryContextSnapshot` (pre-story world hash + entity state), 10 unit test; S22-T02 narrative event extraction + evidence validation, T03 world commit rule engine, T04 transactional world commit + world versioning, T05 event sourcing + rollback/compensation, T06 E2E story advance → committed world change, T07 backlog validation test plan execution; closes S10 limitation)
