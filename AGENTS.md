...
- Sprint 21 — Stability, Performance and Scale [devam ediyor] (spec: `docs/07-delivery/lumi/sprint-21/SPRINT_SPEC.md`; S21-T01 load harness scaffold [committed `codex/sprint-21-load`]: `apps/web/tests/load/session-advance.load.test.ts` + multi-project `vitest.config.ts` (ui jsdom + load node); S21-T02 SLIs + CI gate ✅: non-blocking `pnpm --filter @lumi/web test:load` step in `ci.yml`; S21-T03 memory/CPU baseline ✅: `docs/07-delivery/lumi/sprint-21/S21_T03_MEMORY_CPU_BASELINE.md`; S21-T04 seed snapshot ✅: `packages/story/tests/fixtures/load-seed.fixtures.ts` (synthetic, no PII); S21-T05 final regression gate (wire `s21-t01-report.json` thresholds into CI as soft-failure gate) pending)
